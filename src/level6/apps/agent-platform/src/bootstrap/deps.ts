 ...
 */
import { postgres, redis, queue, rateLimit, OpenAi, appAuth, userSystem, cors, errorResponse, ops, WEIFUWU_USER_SCHEMA } from '../../../../index.ts'
...
export async function buildAppDeps(app: Router<AppCtx>): Promise<PlatformDeps> {
  ...
  if (sysEmails.length) {
    ...
    if (process.env.WF_TEST_HOOKS === '1' && process.env.ADMIN_TEST_PASSWORD) {
      const { hashPassword } = await import('../../../../index.ts')

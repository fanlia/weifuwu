...
import { postgres, WEIFUWU_USER_SCHEMA } from '../../../index.ts'
...
async function cleanTestData(): Promise<void> {
  await (pg as any).orm.query.delete('sandboxes').where({ app_id: { eq: TEST_APP } }).run().catch(() => {})
  // 清理测试容器
...
before(async () => {
  ...
  await pg.migrateModule('test-app-ext', APP_EXT_SCHEMA)
  const [anyUser] = await pg.orm.query.from('_weifuwu_users').select('id').limit(1).run()
  const ownerId = anyUser?.id ?? '00000000-0000-0000-0000-000000000000'
  await pg.orm.query.insert('_weifuwu_apps').rows([{ id: TEST_APP, slug: 'sandbox-test', name: '沙盒测试租户', owner_user_id: ownerId, sandbox_quota: 10 }]).onConflict('id', true, { sandbox_quota: 10 }).run()
  await pg.orm.query.insert('departments').rows([{ id: TEST_DEPT, app_id: TEST_APP, name: '沙盒测试部门', is_dm: false }]).run()
...
test('T-M1a: 并发 ensure 去重——10 并发同部门 → 1 容器全成功（P0-2）', { skip: !HAS_DOCKER, timeout: 20_000 }, async () => {
  ...
  // 记录唯一 + 容器唯一
  const rows = await (pg as any).orm.query.from('sandboxes').where({ department_id: { eq: TEST_DEPT }, status: { ne: 'terminated' } }).run()
...
test('T-M1b: busy 豁免——exec 期间 reconcile 不回收（P0-1）', { skip: !HAS_DOCKER, timeout: 20_000 }, async () => {
  ...
  const [row] = await (pg as any).orm.query.from('sandboxes').where({ department_id: { eq: TEST_DEPT }, status: { ne: 'terminated' } }).run()
  assert.ok(row, '记录应存在')
  ...
  await m.reconcile()
  const [row2] = await (pg as any).orm.query.from('sandboxes').where({ id: { eq: id } }).run()
...
test('T-M2a: 状态机全路径——requested→running→stopped→running→terminated', { skip: !HAS_DOCKER, timeout: 20_000 }, async () => {
  ...
  let [r1] = await (pg as any).orm.query.from('sandboxes').where({ id: { eq: row.id } }).run()
  assert.equal(String(r1.status), 'running')
  // idle 超时 → stopped（两级回收第一级）
  await new Promise(r => setTimeout(r, 900))
  await m.reconcile()
  ;[r1] = await (pg as any).orm.query.from('sandboxes').where({ id: { eq: row.id } }).run()
...
test('T-M2d: per-app 配额——超限创建抛明确错误（409 语义）', { skip: !HAS_DOCKER }, async () => {
  ...
  await (pg as any).orm.query.insert('_weifuwu_apps').rows([{ id: quotaApp, slug: 'sandbox-quota-test', name: '配额测试租户', owner_user_id: '00000000-0000-0000-0000-000000000099', sandbox_quota: 1 }]).onConflict('id', true, { sandbox_quota: 1 }).run()
  const exe = makeSandbox()
  const m = makeManager(exe)
  ...
  await (pg as any).orm.query.delete('sandboxes').where({ app_id: { eq: quotaApp } }).run()
})
...
test('T-M5-2: 池内存预算——超预算驱逐非 busy 最旧 → 仍超抛明确错误', { skip: !HAS_DOCKER }, async () => {
  ...
  await (pg as any).orm.query.insert('_weifuwu_apps').rows([{ id: budgetApp, slug: 'sandbox-budget-test', name: '预算测试租户', owner_user_id: '00000000-0000-0000-0000-000000000099', sandbox_quota: 5 }]).onConflict('id', true, { sandbox_quota: 5 }).run()
  await (pg as any).orm.query.delete('sandboxes').where({ app_id: { eq: budgetApp } }).run()
  const exe = makeSandbox()
  ...
  const [a1] = await (pg as any).orm.query.from('sandboxes').select('status').where({ id: { eq: r1.id } }).run()
  assert.equal(String(a1.status), 'terminated', '最旧的 A 应被驱逐')
  const [a2] = await (pg as any).orm.query.from('sandboxes').select('status').where({ id: { eq: r2.id } }).run()

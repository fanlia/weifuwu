...
import { RedisPool } from '../../../level5/server/db/redis/pool.ts'
import { MemoryRedis } from '../../../level5/server/db/memory-redis.ts'
...
describe('scheduler delayed tasks (real redis)', () => {
  ...
  it('崩溃恢复：schedule 后未消费（未启动守护），重启后到期任务仍执行', async () => {
    ...
    const conn = await import('../../../level5/server/db/redis/connection.ts').then((m) => m.RedisConnection)

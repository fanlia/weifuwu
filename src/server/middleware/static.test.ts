...
describe('serveStatic', () => {
  ...
  it('S8: Range bytes=0-4 → 206 + Content-Range + 截取内容', async () => {
    const handler = serveStatic(tmpDir)
    const res = await handler(new Request('http://localhost/hello.txt', { headers: { Range: 'bytes=0-4' } }), { params: {}, query: {} } as any)
  ...
  it('S8: Range 开区间 bytes=6- → 从 6 到末尾', async () => {
    const handler = serveStatic(tmpDir)
    const res = await handler(new Request('http://localhost/hello.txt', { headers: { Range: 'bytes=6-' } }), { params: {}, query: {} } as any)
    assert.equal(res.status, 206)
  ...
  it('S8: 后缀 Range bytes=-5 → 末 5 字节', async () => {
    const handler = serveStatic(tmpDir)
    const res = await handler(new Request('http://localhost/hello.txt', { headers: { Range: 'bytes=-5' } }), { params: {}, query: {} } as any)
    assert.equal(res.status, 206)
    assert.equal(await res.text(), 'world')
  })
  ...
  it('S8: 越界 Range → 416 + Content-Range bytes */size', async () => {
    const handler = serveStatic(tmpDir)
    const res = await handler(new Request('http://localhost/hello.txt', { headers: { Range: 'bytes=99-' } }), { params: {}, query: {} } as any)
  ...
  it('S8: 预压缩 .gz 存在 + Accept-Encoding: gzip → 服务压缩产物（Content-Type 保留原类型）', async () => {
    ...
    const handler = serveStatic(tmpDir)
    const res = await handler(new Request('http://localhost/hello.txt', { headers: { 'Accept-Encoding': 'gzip' } }), { params: {}, query: {} } as any)
  ...
  it('S8: br 优先于 gz', async () => {
    ...
    const handler = serveStatic(tmpDir)
    const res = await handler(new Request('http://localhost/hello.txt', { headers: { 'Accept-Encoding': 'gzip, br' } }), { params: {}, query: {} } as any)
  ...
  it('S8: 预压缩产物陈旧（mtime 早于原文件）→ 回退原文件（防陈旧内容）', async () => {
    ...
    const handler = serveStatic(tmpDir)
    const res = await handler(new Request('http://localhost/stale.txt', { headers: { 'Accept-Encoding': 'gzip' } }), { params: {}, query: {} } as any)

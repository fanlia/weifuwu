...
import { h, Fragment } from './engines/vdom4/jsx.ts'
...
test('vdom4：Portal 渲染到 #__wf_portal（浮层基础）', async () => {
  const root = mkRoot()
  const { createPortal } = await import('./engines/vdom4/jsx.ts')
  let show = true
  const App = (_init: Record<string, unknown>, ctx: any) => {
...
test('vdom4：usePopup 组件（popup.portal——打开/关闭/外部点击）——迁移试点', async () => {
  const root = mkRoot()
  const { usePopup } = await import('./hooks/popup.ts')
...
test('vdom4 SSR：命令 → HTML → 客户端吸收（路径 id 精确匹配——DOM 引用保持 + 交互正常）', async () => {
  const { renderToCommands, commandsToHtml } = await import('./engines/vdom4/ssr.ts')
  let count = 0
  const App = (_init: Record<string, unknown>, ctx: any) => {
    return () => h('div', { id: 'app' }, [

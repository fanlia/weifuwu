...
import { h, mount, patch, stream, evKey } from './vdom3/index.ts'
...
test('组件：挂载（COMP_MOUNT）→ 更新（复用实例状态保持）→ 卸载（COMP_UNMOUNT）', async () => {
  ...
  const { buildVNode } = await import('./vdom3/build.ts')
  const built = await buildVNode(tree, {})
  const { mount } = await import('./vdom3/index.ts')
  mount(built, root)
  ...
  const { patch } = await import('./vdom3/index.ts')
...
test('调度：同 tick 多次 render → 合并为一次更新（renderFn 不重复执行）', async () => {
  const { Scheduler } = await import('./vdom3/scheduler.ts')
...
test('createRoot：组件 ctx.render → 调度重渲染（内部状态更新 → DOM 更新）', async () => {
  const { createRoot, h } = await import('./vdom3/index.ts')
  const root = mkRoot()
  let count = 0
...
test('回放：DOM = fold(事件流)——重放事件序列 → 结果与原始渲染同构', async () => {
  const { replay } = await import('./vdom3/replay.ts')
  stream.reset()
  const root = mkRoot()
...
test('取消：undo 应用逆操作——INSERT→REMOVE、PROP/TEXT_UPDATE→恢复 prev', async () => {
  const { undo } = await import('./vdom3/replay.ts')
  const { NodeRegistry } = await import('./vdom3/registry.ts')
  stream.reset()
  const root = mkRoot()
  const { mount, patch } = await import('./vdom3/index.ts')
  ...
  undo(events, 2, (await import('./vdom3/render.ts')).registry)
...
test('断言：expectEventSequence——渲染 = 事件序列（精确断言）', async () => {
  const { expectEventSequence, eventsOf } = await import('./vdom3/replay.ts')
  stream.reset()
  const root = mkRoot()
  const { mount } = await import('./vdom3/index.ts')
...
test('路由：navigate → ROUTE_CHANGE 事件 → 页面挂载（全链路事件流）', async () => {
  const { createRouter } = await import('./vdom3/router.ts')
  stream.reset()
  const root = mkRoot()
...
test('SSR：renderToEvents 生成事件流 → serialize/deserialize → replay 重建 DOM（与 mount 同构）', async () => {
  const { renderToEvents, serializeEvents, deserializeEvents } = await import('./vdom3/ssr.ts')
  const { replay } = await import('./vdom3/replay.ts')
  // 组件树（含文本/属性/嵌套）
...
test('SSR：组件挂载事件（COMP_MOUNT）在服务端事件流中（可审计）', async () => {
  const { renderToEvents } = await import('./vdom3/ssr.ts')
...
test('流式渲染：AsyncGenerator 逐事件 → 客户端逐事件 apply → 与完整回放同构', async () => {
  const { renderToEventStream } = await import('./vdom3/ssr.ts')
  const { applyEvent } = await import('./vdom3/replay.ts')
  const { NodeRegistry } = await import('./vdom3/registry.ts')
  const Page = async (_init: any, _ctx: any) => {
...
test('同步：A 渲染 + 交互 → 事件日志 → B 增量镜像（DOM 同构）', async () => {
  const { createSync } = await import('./vdom3/sync.ts')
  const { NodeRegistry } = await import('./vdom3/registry.ts')
  // 全局流隔离（测试独立——避免跨测试残留事件）
  const { stream: globalStream } = await import('./vdom3/events.ts')
  globalStream.reset()
  // 共享事件日志（模拟：一端记录——多端订阅）
  const log: any[] = []
  ...
  await new Promise((r) => setTimeout(r, 20))
  // 同步初始事件（挂载后日志有内容）
  log.push(...(await import('./vdom3/events.ts')).stream.events().slice())
  ...
  const streamMod = await import('./vdom3/events.ts')
...
test('MOVE：keyed 列表重排 → MOVE 事件（非 REMOVE+CREATE）→ DOM 顺序正确', async () => {
  const { stream: gs } = await import('./vdom3/events.ts')
  gs.reset()
  let order = ['a', 'b', 'c']
  const App = async (_init: any, ctx: any) => {
    const rerender = () => ctx.render()
...
test('MOVE：事件流回放含 MOVE（重排可传输）+ undo 恢复原顺序', async () => {
  ...
  const { replay, undo } = await import('./vdom3/replay.ts')
  const { NodeRegistry } = await import('./vdom3/registry.ts')
  const target = document.createElement('div')
  document.body.appendChild(target)
...
test('事件流：环形缓冲——溢出保留最近 max 条（最旧丢弃）+ 顺序正确', async () => {
  const { createEventStream } = await import('./vdom3/events.ts')
...
test('Portal：渲染到 #__wf_portal（父节点位置无内容）+ 更新 + 卸载', async () => {
  const { createPortal, ensurePortalContainer } = await import('./vdom3/index.ts')
  let open = true
  const App = async (_init: any, ctx: any) => {
    const rerender = () => ctx.render()
...
test('Portal：嵌套 portal 随外层关闭一起清理（NavMenu 嵌套子菜单幽灵面板）', async () => {
  const { createPortal } = await import('./vdom3/index.ts')
  const App = async (_init: any, ctx: any) => {
    let outerOpen = true
    let innerOpen = true
    return async () => h('div', { id: 'main' }, [
...
test('Portal：事件流可回放（INSERT parent=portal:key——replay 重建浮层）', async () => {
  const { createPortal, replay } = await import('./vdom3/index.ts')
  const { stream: gs } = await import('./vdom3/events.ts')
  gs.reset()
  const App = async (_init: any, _ctx: any) => async () =>
    h('div', {}, [
...
test('hooks shim：useExternal（createStore 共享状态）在 vdom3 组件运行', async () => {
  const { createV3Ui } = await import('./vdom3/ui.ts')
...
test('useChat：会话创建 + 流式发送（token 累积→notify→订阅组件重渲染）+ 卸载 dispose', async () => {
  const { useChat } = await import('./hooks/chat.ts')
...
test('SSR 端到端：renderToEvents → eventsToHtml（首帧 HTML）→ 客户端 replay（重建——零 DOM 猜测）', async () => {
  const { renderToEvents, eventsToHtml, serializeEvents, deserializeEvents } = await import('./vdom3/ssr.ts')
  const { replay } = await import('./vdom3/replay.ts')
  const Page = async (_init: any, _ctx: any) => {
...
test('audit：开发期不变量——重复 data-v3-id 检测（patch 泄漏）+ 顺序错位检测', async () => {
  const { auditAfterRender } = await import('./vdom3/audit.ts')
...
test('入口形态：vdom2 中间件链复用（api/auth/i18n 注入 → vdom3 ctx）→ 页面组件消费', async () => {
  const { api } = await import('./middleware/api.ts')
  const { createRouter } = await import('./vdom3/router.ts')
  const root = document.createElement('div')
  document.body.appendChild(root)
  ...
  ctx = await (await import('./middleware/auth.ts')).auth({
...
test('命令式 confirm（vdom3——createRoot 挂载 Confirm 组件 + Modal portal 退场）', async () => {
  const { v3Confirm } = await import('./vdom3/commands.ts')
  const root = document.createElement('div')
  document.body.appendChild(root)
  let ctx: any = {}
  ctx = v3Confirm()(ctx)
...
test('命令式 toast（v3——createRoot 挂载 Toast 组件）', async () => {
  const { v3Toast } = await import('./vdom3/commands.ts')
  let ctx: any = {}
  ctx = v3Toast()(ctx)
...
test('命令式 notification（v3——createRoot 挂载 Notification 组件——队列 + 自动消失）', async () => {
  const { v3Notification } = await import('./vdom3/commands.ts')
  let ctx: any = {}
...
test('事件流断言：Modal 关闭应产生 RENDER + REMOVE（组件级 update 的移除可观测）', async () => {
  const { stream: gs } = await import('./vdom3/events.ts')
  const { __resetPopupLockState } = await import('./hooks/popup.ts')
...
test('usePopup 滚动跟随：scroll → rAF 重算坐标 → 组件重渲染（portal 重读 pos 更新 style）', async () => {
  const { createV3Ui } = await import('./vdom3/ui.ts')
  const h = (await import('./vdom3/jsx.ts')).h
  let renders = 0
...
test('事件代理：hooks 全局监听统一（addGlobalListener——聚合注册/退订 + 事件流可观测）', async () => {
  const { stream: gs } = await import('./vdom3/events.ts')
  const { resetDelegation, addGlobalListener } = await import('./vdom3/delegate.ts')
  resetDelegation()
  gs.reset()
...
test('事件代理：removeDelegationRoot 移除挂载点监听（removeEventListener 配对——卸载无残留）', async () => {
  const { resetDelegation, ensureDelegationRoot, removeDelegationRoot } = await import('./vdom3/delegate.ts')
  resetDelegation()
  const { createRoot } = await import('./vdom3/root.ts')
  const root = document.createElement('div')
  document.body.appendChild(root)
...
test('事件代理 once：动画监听分发一次后自动解绑（EVENT_UNBIND 可观测——与 addEventListener {once} 等价）', async () => {
  const { stream: gs } = await import('./vdom3/events.ts')
  const { resetDelegation, bindElementListener } = await import('./vdom3/delegate.ts')
  resetDelegation()
  gs.reset()
  const root = document.createElement('div')
  document.body.appendChild(root)
...
test('事件代理：不冒泡事件（img error）用捕获监听——onError 分发（Img fallback 回归）', async () => {
  const { h, createRoot, stream, evKey } = await import('./vdom3/index.ts')
...
test('组件副作用事件流：ref:mount + effect:animate/lock/unlock/focus/scroll 可观测', async () => {
  ...
  gs.reset()
  const { animateOut } = await import('./motion.ts')
...
test('用户文本操作事件流：text:input（输入/组合）+ text:select（选区）可观测', async () => {
  const { stream: gs } = await import('./vdom3/events.ts')
  const { resetDelegation } = await import('./vdom3/delegate.ts')
  resetDelegation()
  gs.reset()
  const root = document.createElement('div')
  document.body.appendChild(root)
...
test('组件索引 O(1) 定位：注册/复用/移除一致性（updateComponent 不经 DFS）', async () => {
  const { stream: gs } = await import('./vdom3/events.ts')
  const { resetCompIndex, getIndexedComponent } = await import('./vdom3/comp-index.ts')
  resetCompIndex()
  gs.reset()
  const root = document.createElement('div')
  document.body.appendChild(root)
...
test('DOM↔事件流对照审计：注入绕过（直接 removeChild）→ audit warn（无事件流不渲染守护）', async () => {
  ...
  const root2 = document.createElement('div')
  document.body.appendChild(root2)
  const { createRoot: createRoot2 } = await import('./vdom3/root.ts')
...
test('app 节点：多应用加载——注册表/工厂/边界事件/同流全链路/卸载', async () => {
  const { stream: gs } = await import('./vdom3/events.ts')
  const { registerApp, resetAppRegistry, getAppFactory } = await import('./vdom3/app.ts')
  resetAppRegistry()
  gs.reset()
  const root = document.createElement('div')
  document.body.appendChild(root)
  const { createRoot } = await import('./vdom3/root.ts')
  const { App, h } = await import('./vdom3/index.ts')
...
test('阶段 1：SSR 空洞——占位序列化 + 客户端 replay 同构', async () => {
  const { renderToEvents, eventsToHtml } = await import('./vdom3/ssr.ts')
  const { replay } = await import('./vdom3/replay.ts')
  let open = false
...
test('阶段 2：组件输出 Fragment 在数组中间——切换不漂移（多节点范围锚）', async () => {
  ...
  document.body.appendChild(root)
  const { createRoot } = await import('./vdom3/root.ts')
  const { Fragment } = await import('./vdom3/types.ts')
  let show = true
...
test('阶段 4：portal 槽切换不触发动态数组检测（[children, portal] 模式——HoverCard 误报）', async () => {
  const root = document.createElement('div')
  document.body.appendChild(root)
  const { createRoot, createPortal } = await import('./vdom3/index.ts')
  const warns: string[] = []
  const ow = console.error
  console.error = (...a: any[]) => { if (String(a[0]).includes('[vdom3/audit] 动态数组')) warns.push(String(a[0])); ow(...a) }
  let open = false
...
test('阶段 4：单子节点条件渲染（cond ? <X/> : null）不触发检测（ColorPicker check Icon 误报）', async () => {
  const root = document.createElement('div')
  document.body.appendChild(root)
  const { createRoot } = await import('./vdom3/index.ts')
  const warns: string[] = []
  const ow = console.error
  console.error = (...a: any[]) => { if (String(a[0]).includes('[vdom3/audit] 动态数组')) warns.push(String(a[0])); ow(...a) }
  let sel = true
...
test('阶段 3：portal open/close 事件（弹层开合可观测）', async () => {
  ...
  document.body.appendChild(root)
  const { createRoot } = await import('./vdom3/root.ts')
  const { Portal } = await import('./vdom3/types.ts')
  let open = false
  const App = async (_init: any) => async () => h('div', {}, [
...
test('阶段 4：监听残留 dev warn；正常清理无残留', async () => {
  ...
  document.body.appendChild(root)
  const { createRoot } = await import('./vdom3/root.ts')
  const { bindDelegated, listenerCount } = await import('./vdom3/delegate.ts')
  const warns: string[] = []
  const ow = console.warn
...
test('UI-3：selfId + render([\'id\']) 跨组件精准刷新——只重渲染目标组件', async () => {
  const { resetSemanticService } = await import('./services/hook-env.ts')
  ...
  // 服务层同步语义直接断言）
  const { registerSemanticId } = await import('./services/hook-env.ts')
...
test('P3：unmount dispose 协议——组件钩子 + ref(null) + 事件解绑 + portal 清空', async () => {
  ...
  document.body.appendChild(root)
  const { createRoot } = await import('./vdom3/root.ts')
  const { listenerCount } = await import('./vdom3/delegate.ts')
  let hookCalls = 0
  let refCleans = 0

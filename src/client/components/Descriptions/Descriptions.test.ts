...
describe('Descriptions', () => {
  it('渲染 dl 结构（语义）', async () => {
    const vnode = await renderVNode(Descriptions, { items }, createTestCtx())!
  ...
  it('每项含 dt(label) + dd(value)', async () => {
    const vnode = await renderVNode(Descriptions, { items }, createTestCtx())!
  ...
  it('column 控制栅格列数类', async () => {
    const vnode = await renderVNode(Descriptions, { items, column: 2 }, createTestCtx())!
    assert.match(vnode.props.class, /wf-descriptions--2/)
  })

  it('bordered 类', async () => {
    const vnode = await renderVNode(Descriptions, { items, bordered: true }, createTestCtx())!
    assert.match(vnode.props.class, /wf-descriptions--bordered/)
  })

  it('span 跨列（grid-column）', async () => {
    const vnode = await renderVNode(Descriptions, { items: [{ label: 'a', value: '1', span: 2 }], column: 2 }, createTestCtx())!
    const item = vnode.props.children[0]
    assert.match(item.props.style.gridColumn, /span 2/)
  })

  it('VNode value 支持（自定义渲染）', async () => {
    const vnode = await renderVNode(Descriptions, { items: [{ label: 'x', value: { tag: 'strong', props: {}, children: '加粗' } }] }, createTestCtx())!

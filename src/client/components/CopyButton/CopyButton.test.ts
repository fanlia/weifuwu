...
describe('CopyButton', () => {
  ...
  it('renders button with copy icon', async () => {
    ...
    const vnode = await render({ value: 'text' })
  ...
  it('copies value via ctx.browser.copyText on click', async () => {
    ...
    const vnode = await render({ value: '你好世界' })
  ...
  it('calls onCopied after success', async () => {
    ...
    await (await render({ value: 'x', onCopied: () => { copied = true } })).props.onClick()
  ...
  it('renders label when provided', async () => {
    const render = await mount(CopyButton, { value: 'x' }, makeCtx())!
    const vnode = await render({ value: 'x', label: '复制' })
  ...
  it('iconOnly renders no text', async () => {
    ...
    const vnode = await render({ value: 'x', iconOnly: true })
...
it('复制失败（copyText  reject）不崩溃且无成功态（边界）', async () => {
  ...
  const vnode = await factory({ value: 'x' })
...
it('size/variant 变体类', async () => {
  ...
  const vnode = await render({ value: 'x', size: 'sm', variant: 'ghost' })

...
function typeClass(v) {
    if (v === null)
        return 'wf-json-null';
    if (typeof v === 'string')
        return 'wf-json-string';
    if (typeof v === 'number')
        return 'wf-json-number';
    if (typeof v === 'boolean')
        return 'wf-json-boolean';
    return '';
}
...
export const JSONViewer = async (_init, ctx) => {
    ...
    const copyPath = (path, value, onCopy) => {
        if (onCopy) {
            onCopy(path, value);
            return;
        }
    ...
    return async (props) => {
        ...
        // 顶层：键行（depth=0）
        const renderValue = (v, path, depth, key) => {
                ...
                if (isCollapsed) {
                    ...
                    return h('div', {
                        ...
                        h('button', {
                            ...
                            'aria-label': '展开',
                            tabIndex: -1,
                            onClick: (e) => { e.stopPropagation(); toggle(path, isCollapsed); },
                        ...
                        h('button', {
                            ...
                            'aria-label': `复制 ${path}`,
                            tabIndex: -1,
                            onClick: (e) => { e.stopPropagation(); copyHere(path, v, e); },
            ...
            root = isCollapsed
                ? h('div', {
                    ...
                    onKeyDown: (e) => {
                        if (e.key === 'Enter' || e.key === ' ') {
                            e.preventDefault();
                            toggle(rootName, isCollapsed);
                        }
                    },
                }, [
                    h('button', { class: 'wf-json-toggle', 'aria-label': '展开', tabIndex: -1, onClick: (e) => { e.stopPropagation(); toggle(rootName, isCollapsed); } }, h(Icon, { name: 'chevron-right', size: 10 })),
                ...
                : (Array.isArray(data)
                    ? h('div', { class: 'wf-json-node', 'data-path': rootName }, [
                        h('div', {
                            ...
                            h('button', { class: 'wf-json-toggle', 'aria-label': '收起', tabIndex: -1, onClick: (e) => { e.stopPropagation(); toggle(rootName, isCollapsed); } }, h(Icon, { name: 'chevron-down', size: 10 })),
                    ...
                    : h('div', { class: 'wf-json-node', 'data-path': rootName }, [
                        h('div', {
                            ...
                            h('span', { class: 'wf-json-node-summary' }, `Object(${Object.keys(data).length})`),
                        ]),

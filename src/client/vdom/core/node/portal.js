...
export const Portal = Symbol('vdom-portal');
...
/** Portal vnode 判定 */
export function isPortal(v) {
    return v.type === Portal;
}
/** portal 容器 id（按 key——语义化——组件名） */
export function portalContainerId(portalKey) {
    return `${PORTAL_CONTAINER_ID}-${portalKey}`;
}
/** 创建 Portal vnode（usePopup 内部调用——纯数据） */
export function createPortal(children, portalKey) {
    return {
        type: Portal,

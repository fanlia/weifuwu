...
export function pathId(parent, i) {
    return `${parent}.${i}`;
}
/** 可序列化属性面（服务端吐 HTML 用）——事件/ref/函数值排除 */
export function serializableAttrs(props) {
    const out = {};
...
export async function renderNative(vn, id, parent, ref, emitCommand, sink) {
    emitCommand({ op: 'create', id, tag: vn.type, attrs: serializableAttrs(vn.props) });
    ...
    if (typeof refFn === 'function')
        emitCommand({ op: 'ref', id, fn: refFn });

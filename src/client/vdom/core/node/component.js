...
export function disposeAllComponents(registry) {
    for (const id of [...registry.keys()].reverse())
        disposeComponent(id, registry);
}
/** 渲染一个组件（mount + renderFn——可 await——输出经 sink 递归 emit） */
export async function renderComponent(vn, parent, index, ref, compId, sharedCtx, registry, sink) {
    const factory = vn.type;
    // 实例记录（同位置同类型复用——工厂不重跑）
    ...
    if (!rec) {
        ...
        const hookSeq = { n: 0 };
        const instCtx = Object.create(sharedCtx);
        ...
        instCtx.ui = createUi({
            ...
            getHookState: (idx) => hookStates.get(idx),
...
export function disposeComponent(id, registry) {
    ...
    for (const fn of rec.onUnmounts.reverse()) {
        try {
            ...
            console.error('[vdom] onUnmount:', e);
        }
    }
    registry.delete(id);
}

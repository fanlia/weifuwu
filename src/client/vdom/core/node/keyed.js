...
export function isKeyed(v) {
    return keyOf(v) !== null;
}
/** 列表 keyed 分类（空列表 = all-unkeyed——无身份可言） */
export function listKind(items) {
    let hasKey = false;
...
export function planKeyedDiff(oldItems, newItems) {
...
export function detectMissingKey(items, context) {
    if (items.length < 2)
        return;
    const hasComponent = items.some((i) => typeof i?.type === 'function');
    const hasMissing = items.some((i) => !isKeyed(i) && typeof i?.type === 'function');
    if (hasComponent && hasMissing) {
        console.warn(`[vdom] ${context}：列表含无 key 的组件项（长度 ${items.length}）——` +

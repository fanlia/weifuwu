...
test('ref 引用变化：旧 ref 退 null + 新 ref 接 el（diff 重绑定）', () => {
  ...
  applyRef(e, r1)
  applyRef(e, r2, r1)
  expect(calls, '旧 ref 退 null 后新 ref 接 el').toEqual(['r1:el', 'r1:null', 'r2:el'])
})
...
test('ref 同引用：不重复退旧（diff 幂等）', () => {
  ...
  applyRef(e, ref)
  applyRef(e, ref, ref)
  expect(calls, '同引用——prev === next——不退旧不重绑').toEqual(['el'])
})

test('ref 常量：REF_KEY = ref（setProp 分发键）', () => {
  expect(REF_KEY).toBe('ref')
})
...
test('RefRegistry：注册 → mount 触发（insert 后查表）；unmount ref(null) 子树', () => {
  ...
  reg.set('root.0', fn)
  reg.mount('root.0', el)
  expect(calls, 'mount 查表触发').toEqual(['mount:x'])
  reg.unmount('root.0')
  expect(calls, 'unmount → ref(null)').toEqual(['mount:x', 'unmount'])
  // 卸载后表已删——再 mount 不触发
  reg.mount('root.0', el)
  expect(calls).toEqual(['mount:x', 'unmount'])
})
...
test('RefRegistry：子树前缀匹配（unmount 整棵子树）', () => {
  ...
  reg.unmount('root.0')
  expect(calls, '子树 ref(null) 全部').toEqual(['a', 'b'])
  reg.mount('root.1', {} as HTMLElement)
  expect(calls, '兄弟子树不受影响').toEqual(['a', 'b', 'c'])
})
...
test('RefRegistry：set prev 重绑——旧引用退 null + 新引用 mount', () => {
  ...
  expect(calls, 'prev 旧引用退 null').toEqual(['r1:el', 'r1:null'])
  reg.mount('root.0', el)
  expect(calls, '新引用 mount').toEqual(['r1:el', 'r1:null', 'r2:el'])
})
...
test('RefRegistry：move 前缀重映射（节点移动——表跟随）', () => {
  ...
  expect(calls, '重映射后新 id 查表命中').toEqual(['mount'])
})

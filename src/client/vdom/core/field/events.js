...
export class EventRegistry {
    ...
    ensureRoot(event) {
        if (this.rootTypes.has(event))
            return;
        this.doc.addEventListener(event, this.dispatchBound, { capture: true });
    ...
    dispose() {
        for (const t of this.rootTypes) {
            this.doc.removeEventListener(t, this.dispatchBound, { capture: true });
        }

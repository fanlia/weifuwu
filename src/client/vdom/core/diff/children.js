...
import { listKind, keyOf, detectMissingKey, isKeyed } from '../node/keyed.ts';
...
export async function diffChildren(oldV, newV, id, emit, emitCommand, ctx, registry) {
...
export async function diffChildrenItems(oldCs, newCs, id, emit, emitCommand, ctx, registry, shapeChanged = false) {
...
async function removeOldSlot(oldC, parent, cid, emitCommand) {
    if (oldC === null || oldC === undefined || typeof oldC === 'boolean')
        return;
    const oldVn = oldC;
...
async function diffSlot(oldC, newC, parent, index, ref, cid, emit, emitCommand, ctx, registry) {
    ...
    if (kindOf(oldC) === kindOf(newC) && typeof oldC !== 'string' && typeof oldC !== 'number') {
        await diffSame(oldC, newC, parent, index, ref, emit, emitCommand, ctx, registry);
        return;
    }
    ...
    if (t) {
        await t(oldC, newC, {
            ...
            oldCompId: typeof oldC?.type === 'function' ? cid : undefined,
        });
    }
}
// ── keyed 列表策略（细节模块） ──
// ── keyed 列表策略（细节模块） ──
export function hasKeyed(items) {
...
export async function diffKeyedChildren(oldCs, newCs, parent, emit, emitCommand, ctx, registry) {
    // 旧 key → 旧索引（身份映射）
    const oldIdxByKey = new Map();
    oldCs.forEach((c, i) => { const k = keyOf(c); if (k !== null)
        oldIdxByKey.set(k, i); });
    const newKeys = new Set(newCs.map((c) => keyOf(c)).filter((k) => k !== null));
    // 1. **真移除**（不在新列表——unmount（组件）+ remove）
    for (const [k, oldIdx] of oldIdxByKey) {
        if (!newKeys.has(k)) {
            if (typeof oldCs[oldIdx].type === 'function') {
    ...
    //      （实例复用——状态保持）
    const keptOld = oldCs.map((c) => keyOf(c)).filter((k) => k !== null && newKeys.has(k));
    // **顺序检测只比较新旧共有的 key**（新增/移除不参与——否则增项/删项
    // 被误判冲突重建——remove 全部 + 重建——非组件项引用丢失/焦点丢失——
    // diff 本质受损——真实 bug）
    const keptNew = newCs.map((c) => keyOf(c)).filter((k) => k !== null && oldIdxByKey.has(k));
    let subseq = true;
    {
        let p = 0;
    ...
    if (!subseq) {
        ...
        for (let i = 0; i < newCs.length; i++) {
            const newC = newCs[i];
            const k = keyOf(newC);
            if (k !== null && typeof newC.type === 'function') {
                // **全量渲染（节点已删——无对照——sink = emit）**——组件实例
                // .k{key} 复用（状态保持）
                const keyedId = `${parent}.k${k}`;
                const isNew = await renderComponent(newC, parent, i, r, keyedId, ctx, registry, emit);
                if (isNew)
                    emitCommand({ op: 'mount', compId: keyedId });
            }
            else {
    ...
    for (let i = 0; i < newCs.length; i++) {
        ...
        if (k !== null) {
            ...
            if (oldIdx === undefined) {
            ...
            else if (typeof newC.type === 'function') {
                ...
                await diffSame(oldCs[oldIdx], newC, parent, i, lastRef, emit, emitCommand, ctx, registry);
            }
        }
        else {
            // 无 key 项（混合数组）——重建
...
export async function emitWithKey(v, parent, index, ref, key, emit, emitCommand, ctx, registry) {
    const vn = v;
        ...
        const isNew = await renderComponent(vn, parent, index, ref, keyedId, ctx, registry, async (out, p, i, r) => {
            ...
            if (out === null || out === undefined) {
                if (oldOut !== undefined && oldOut !== null) {
                    ...
                    if (t)
                        await t(oldOut, out, { emit: emitCommand, emitNode: emit, oldId: outId, newId: outId, parent: p, index: i, ref: r });
                }
            ...
            if (oldOut === null) {
                ...
                if (t)
                    await t(null, out, { emit: emitCommand, emitNode: emit, oldId: outId, newId: outId, parent: p, index: i, ref: r });
                return;
            }
            if (oldOut !== undefined && !Array.isArray(oldOut) && !Array.isArray(out)) {
                // 上次输出对照（同实例——精准增量）
                await diffSame(oldOut, out, p, i, r, emit, emitCommand, ctx, registry);
            }
            else {

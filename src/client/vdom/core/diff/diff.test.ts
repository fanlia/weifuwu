...
function harness() {
  ...
  const applier = new CommandApplier(root, document, registry)
...
test('首帧：无旧树 → 全量 build（等价 renderToStream）', async () => {
  ...
  expect(hz.root.querySelector('.a')?.textContent).toBe('hello')
})
...
test('同类型元素：属性更新 + children 递归 patch（就地——不重建）', async () => {
  ...
  expect(div.className, '属性就地更新').toBe('new')
  expect(div.querySelectorAll('span').length).toBe(2)
  expect(div.querySelectorAll('span')[0].textContent, '文本就地 setText 更新').toBe('A')
  expect(div.querySelectorAll('span')[1].textContent, '未变项保持').toBe('b')
})
...
test('文本变化：setText 就地更新（节点不重建——焦点保持前提）', async () => {
  ...
  expect(hz.root.querySelector('div')?.textContent).toBe('v2')
  expect(hz.root.querySelector('div'), '同一 div 节点——不重建').toBe(divBefore)
})
...
test('新增/移除 children：新项插入 + 旧项移除（就地）', async () => {
  ...
  const div = hz.root.querySelector('div')!
  expect(div.querySelectorAll('span').length).toBe(2)
  expect(div.querySelector('.a')?.textContent).toBe('a')
  expect(div.querySelector('.c')?.textContent).toBe('c')
  expect(div.querySelector('.b'), '旧项移除').toBe(null)
})
...
test('组件同类型复用：工厂不重跑——renderFn 重新调用读最新状态', async () => {
  ...
  expect(hz.root.querySelector('.c')?.textContent).toBe('v:1')
  await hz.update(h(Counter, { value: 1 }), h(Counter, { value: 2 }))
  expect(mounts, '工厂不重跑——实例复用').toBe(1)
  expect(renders, 'renderFn 重新调用').toBe(2)
  expect(hz.root.querySelector('.c')?.textContent, 'props 变化 → DOM 更新').toBe('v:2')
})
...
test('异类型转换：element 移除（数组缩短——同构 1:1——就地）', async () => {
  ...
  const div = hz.root.querySelector('div')!
  expect(div.childNodes.length, '新树 1 项 ⟷ DOM 1 节点（长度 1:1——同构）').toBe(1)
  expect(div.querySelector('b'), 'b 移除').toBe(null)
  expect(div.querySelector('span')?.textContent, 'a 保留').toBe('a')
})
...
test('条件渲染：null → 元素（空洞 ↔ 真实节点互换——长度恒定）', async () => {
  ...
  const div = hz.root.querySelector('div')!
  expect(div.childNodes.length, '长度恒定（锚 → b 互换）').toBe(3)
  expect(div.childNodes[1].textContent, '空洞位置被新元素占据').toBe('new')
})
...
test('diff 本质：精准命令流——counter 点击只发文本 setText（不重建/不重发属性）', async () => {
  ...
  expect(hz.root.querySelector('#inc')?.textContent).toBe('count:0')
  ...
  expect(ops, '精准命令流——只修改文本节点').toEqual(['setText', 'done'])
  const setText = cmds[0] as { id: string; value: string }
  expect(setText.id, '文本节点 id 精准定位').toBe('root.0.0.0')
  expect(setText.value).toBe('count:1')
})
...
test('diff 本质：属性变化只发 setProp（不重建元素）', async () => {
  ...
  const ops = cmds.map((c) => (c as { op: string }).op)
  expect(ops, '只发属性更新——元素不重建').toEqual(['setProp', 'done'])
})
...
test('keyed 列表：增/删——组件状态跟随 key（身份复用——不按位置继承）', async () => {
  ...
  expect(hz.root.querySelectorAll('.it')[0].textContent).toBe('a:0')
  // 增：c 加入（a/b 保持）
  await hz.update(list(['a', 'b']), list(['a', 'b', 'c']))
  expect(mounts, '只 mount c——a/b 复用').toBe(3)
  expect(hz.root.querySelectorAll('.it').length).toBe(3)
  // 删：b 移除——a 状态保持
  await hz.update(list(['a', 'b', 'c']), list(['a', 'c']))
  expect(hz.root.querySelectorAll('.it').length).toBe(2)
  expect(hz.root.querySelectorAll('.it')[0].textContent).toBe('a:0')
  expect(hz.root.querySelectorAll('.it')[1].textContent, 'c 顶位——身份跟随 key').toBe('c:0')
})
...
test('keyed 列表：重排——组件状态跟随 key（不按位置继承——身份不漂移）', async () => {
  ...
  expect(items[0].textContent, 'c 首位——状态保持（key 身份）').toBe('c:9')
  expect(items[1].textContent, 'a 状态保持').toBe('a:1')
  expect(items[2].textContent, 'b 状态保持').toBe('b:5')
})
...
test('keyed 列表：重排——组件实例复用（工厂不重跑——状态保持）', async () => {
  ...
  await hz.update(list(['a', 'b', 'c']), list(['c', 'a', 'b']))
  expect(mounts, '重排——组件实例复用（.k{key} 身份——工厂不重跑）').toBe(3)
  const items = hz.root.querySelectorAll('.it')
  expect(items.length).toBe(3)
  expect(items[0].textContent, '新顺序正确').toBe('c')
...
test('A 级检测：长度变化 + 无 key 组件项 → warn 引导声明 key（位置继承漂移防护）', async () => {
  ...
  expect(warns.length, '长度变化 + 无 key 组件 → warn').toBe(1)
...
test('A 级检测：长度一致（无 key）不 warn；长度变化（全 keyed）不 warn', async () => {
  ...
  expect(warns.length, '长度一致——位置身份正确——不 warn').toBe(0)
    // 长度变化（2 → 3——全 keyed）——身份复用——不 warn
    await hz.update(list(['a', 'b']), list(['a', 'b', 'c']))
  expect(warns.length, '全 keyed——身份复用——不 warn').toBe(0)
  } finally {
    console.warn = origWarn
  }
})
...
test('组件输出 null：条件渲染组件切换（div → null → div——占位锚 ↔ 真实节点——X-G4）', async () => {
  ...
  expect(div, '组件输出 div').toBeTruthy()
  // 输出 → null：占位锚替换（同构——长度保持）
  toggle = () => {}
  await hz.update(page(true, toggle), page(false, toggle))
  const container = hz.root.querySelector('div')!
  expect(container.childNodes.length, '长度恒定（div → 锚）').toBe(1)
  expect(container.childNodes[0].nodeType, '组件输出 null → wf-hole 占位锚').toBe(8)
  // null → 输出：锚 → 真实节点（X-G4 恢复）
  await hz.update(page(false, toggle), page(true, toggle))
  expect(hz.root.querySelector('.box')?.textContent, '恢复渲染').toBe('内容')
  expect(container.childNodes.length, '同构保持（锚 → div）').toBe(1)
})
...
test('组件输出 null 精准命令流：div → null 只发 remove + 锚（无重建噪音）', async () => {
  ...
  const ops = cmds.map((c) => (c as { op: string }).op)
  expect(ops, '精准：组件输出 div → null——remove 旧 + 占位锚').toEqual(['remove', 'createAnchor', 'insert', 'done'])
})

test('patch 生命周期：ref 挂载后执行（insert 后 el 已连接）+ 移除时 ref(null) 清理', async () => {
  const reg = createComponentRegistry()
  const root = document.querySelector('#root') as HTMLElement
  const applier = new CommandApplier(root, document, reg)
  const calls: string[] = []
  ...
  expect(calls, 'ref 在挂载后执行（el.isConnected = true）').toEqual(['mount:true'])
  // 移除节点 → ref(null)（卸载清理）
  ...
  expect(calls, '移除 → ref(null)').toEqual(['mount:true', 'unmount'])
})
...
test('patch 生命周期：unmountComp 执行 onUnmounts（组件卸载清理）', async () => {
  ...
  const page = (show: boolean) => h('div', {}, show
    ...
    : [h('div', { class: 'empty' }, '空')])
  await hz.mount(page(true))
  expect(cleaned.length, '挂载时未清理').toBe(0)
  // 异类型转换（component → element——整块让位）→ unmount 命令 → onUnmounts 执行
  await hz.update(page(true), page(false))
  expect(cleaned.sort(), '卸载时 onUnmounts 执行（patch 消费 unmountComp）').toEqual(['item-cleanup', 'page-cleanup'])
  expect(hz.root.querySelector('.empty')?.textContent).toBe('空')
})
...
test('生命周期指令：ref（insert 后挂载完成——el 已连接）/unref（ref(null)）', async () => {
  ...
  const r1 = s1.getReader()
  while (true) { const { value, done } = await r1.read(); if (done) break; applier.apply(value) }
  expect(calls, 'ref 指令：insert 后执行（el 已连接）').toEqual(['mount:true'])
  // 显式 unref 指令 → ref(null)
  applier.apply({ op: 'unref', id: 'root.0.0' })
  expect(calls, 'unref 指令：ref(null) 清理').toEqual(['mount:true', 'unmount'])
})
...
test('生命周期指令：mount（新实例初始化完成）/unmount（onUnmounts 清理）', async () => {
  ...
  await hz.mount(page(true))
  expect(events, '挂载不清理').toEqual([])
  // 组件移除（数组缩短——unmount 指令）→ onUnmounts 执行
  await hz.update(page(true), page(false))
  expect(events, 'unmount 指令：onUnmounts 执行').toEqual(['cleanup'])
})
...
test('生命周期指令：mount 标记实例已挂载（审计配对——patch 消费）', async () => {
  ...
  expect(rec?.mounted, 'mount 指令标记实例已挂载').toBe(true)
})
...
test('keyed 真移除：不在新列表的 key → unmount（onUnmounts 清理）——复用项不清理', async () => {
  ...
  await hz.mount(list(['a', 'b', 'c']))
  await hz.update(list(['a', 'b', 'c']), list(['a', 'c']))
  expect(cleaned, '真移除项 b 卸载清理——复用项 a/c 不清理').toEqual(['un:b'])
  expect(mounts, '复用项工厂不重跑').toBe(3)
})
...
test('混合数组：keyed 项身份复用（状态跟随 key）+ 无 key 项重建', async () => {
  ...
  expect(mounts, 'keyed 项复用——工厂不重跑').toBe(2)
  const items = hz.root.querySelectorAll('span')
  const texts = [...items].map((s) => s.textContent)
  expect(texts.includes('k2:7'), 'k2 状态保持（k2:7）').toBeTruthy()
  expect(texts.includes('k1:3'), 'k1 状态保持（k1:3）').toBeTruthy()
})
...
test('move：keyed 删除顺移——节点移动（DOM 不重建——isConnected 保持）', async () => {
  ...
  expect(after.map((e) => e.id), '新顺序正确').toEqual(['a', 'c'])
  // 移动：同一节点引用（DOM 不重建——焦点/输入状态保持）
  expect(after[1], 'c 节点移动（非重建）').toBe(before[2])
})
...
test('move：子树 id 重映射——子节点引用保持（深层结构移动）', async () => {
  ...
  expect(after[1], 'z 子节点移动保持（子树重映射）').toBe(titles[2])
  expect(after[0].textContent).toBe('t-y')
  expect(after[1].textContent).toBe('t-z')
})
...
test('move 命令流：删除顺移只发 noMove remap（无 create/remove——节点复用）', async () => {
  ...
  const ops = cmds.map((c) => (c as { op: string }).op)
  expect(!ops.includes('create'), '顺移无 create（节点复用）').toBeTruthy()
  const moves = cmds.filter((c) => (c as { op: string }).op === 'move') as Array<{ noMove?: boolean }>
  expect(moves.length, 'c 单顺移（b 移除——a 位置不变）').toBe(1)
  expect(moves[0].noMove, '顺移 = noMove remap（位置自然到位——id 前缀迁移）').toBe(true)
})
...
test('patch 资源释放：remove/done 后 propPrev 表清理（事件旧值引用不残留）', async () => {
  ...
  expect(hz.root.querySelectorAll('button').length).toBe(2)
  expect(hz.root.querySelectorAll('button')[0].textContent).toBe('a')
  expect(hz.root.querySelectorAll('button')[1].textContent).toBe('c')
})
...
test('patch 资源释放：done.full 清理后 propPrev 不残留（全量流重建）', async () => {
  ...
  const r2 = s2.getReader()
  while (true) { const { value, done } = await r2.read(); if (done) break; applier.apply(value) }
  expect(root.querySelector('#b'), '节点已移除').toBe(null)
  ...
  expect(root.querySelector('#b')?.textContent, '重建正常').toBe('x')
})
...
test('组件输出数组 ↔ 单节点：双向转换——旧输出完整清理（残留 bug 修复）', async () => {
  ...
  expect(hz.root.querySelectorAll('span').length, '数组输出（隐式 Fragment）').toBe(2)
  // 数组 → 单节点：旧数组逐项清理（无残留）
  await hz.update(page('multi'), page('single'))
  expect(hz.root.querySelector('.single')?.textContent, '单节点渲染').toBe('单')
  expect(hz.root.querySelectorAll('span').length, '旧数组项完整移除（无 m1/m2 残留）').toBe(0)
  // 单节点 → 数组：反向转换
  await hz.update(page('single'), page('multi'))
  expect(hz.root.querySelectorAll('span').length, '数组恢复').toBe(2)
  expect(hz.root.querySelector('.single'), '单节点移除').toBe(null)
})
...
test('四层集成：build/diff/transform/patch 全职责——命令级验证', async () => {
  ...
  expect(ops1.includes('ref'), 'build 层：ref 指令（挂载完成）').toBeTruthy()
  expect(ops1.includes('mount'), 'build 层：mount 指令（组件初始化）').toBeTruthy()
  expect(ops1.includes('setProp'), 'build 层：setProp（事件代理注册）').toBeTruthy()
  expect((cmds1[cmds1.length - 1] as { op: string }), 'build 层：done.full').toEqual({ op: 'done', full: true })
  expect(root.querySelectorAll('.it').length, 'keyed 列表 2 项').toBe(2)
  expect(refCalls, 'ref 挂载（isConnected）').toEqual(['hd-mount'])
  ...
  expect(ops2, 'diff 层：点赞 → 只 setText（值比较——最小命令集）').toEqual(['setText', 'done'])
  // 无变化 → 无命令（再 diff 相同状态——零噪音）
  const cmds2b = await run(diffStream(pageA(['a', 'b']), pageA(['a', 'b']), {} as Ctx, reg))
  expect(cmds2b, 'diff 层：无变化不发命令').toEqual([{ op: 'done' }])
  ...
  expect(ops3.includes('remove') && ops3.includes('create'), 'transform 层：null → 组件（占位锚让位 + 新侧渲染）').toBeTruthy()
  expect(root.querySelector('.extra'), 'Extra 渲染').toBeTruthy()
  ...
  expect(ops4.includes('mount'), 'keyed 增：新实例 mount').toBeTruthy()
  expect(root.querySelectorAll('.it').length).toBe(3)
  ...
  expect(ops5.includes('remove') && ops5.includes('create'), '循环移位：冲突检测 → 重建路径（move id 覆盖事故的根治）').toBeTruthy()
  const after = [...root.querySelectorAll('.it')]
  expect(after.map((e) => e.querySelector('.t')?.textContent), '重排顺序正确 + 实例复用（a 投票状态保持）').toEqual(['c(0)', 'b(0)', 'a(1)'])
  ...
  expect(after[0].querySelector('.t')?.textContent, 'patch 层：重建后事件代理仍分发（diff 精准 setText）').toBe('c(1)')
  ...
  expect(doc.querySelector('.panel'), '浮层打开（#__wf_portal）').toBeTruthy()
  const cmds6 = await run(diffStream(panelPage(true), panelPage(false), {} as Ctx, reg))
  const ops6 = cmds6.map((c) => (c as { op: string }).op)
  expect(ops6.includes('removePortal'), 'patch 层：浮层关闭 → removePortal（容器清理）').toBeTruthy()
  expect(doc.querySelector('.panel'), '浮层移除').toBe(null)
  ...
  expect(ops7.includes('mount'), 'route 层：新页全量 build（mount 指令）').toBeTruthy()
  expect(root.querySelector('.pageB')?.textContent, '新页渲染').toBe('B页')
  // 卸载顺序（整树替换——LIFO）
  expect(refCalls, 'ref 生命周期（卸载 + 新挂载）').toEqual(['hd-mount', 'hd-unmount', 'hd-mount'])
  expect(events.includes('un:pageA'), 'PageA 卸载').toBeTruthy()
  expect(events.includes('un:item:a'), 'Item 卸载（keyed 实例清理）').toBeTruthy()
})
...
test('diffStream root 异态转换：元素 → 组件（transitionOf root 分支——完整转换）', async () => {
  ...
  expect(hz.root.querySelector('.el'), '首帧元素根').toBeTruthy()
  const Comp = () => () => h('span', { class: 'comp' }, '组件')
  // diff：元素 → 组件（异态——transform 完整转换——旧让位 + 新侧）
  await hz.update(h('div', { class: 'el' }, '元素'), h(Comp, {}))
  expect(hz.root.querySelector('.el'), '旧元素移除（root 异态转换）').toBe(null)
  expect(hz.root.querySelector('.comp'), '新组件渲染').toBeTruthy()
  expect(hz.root.querySelector('.comp')?.textContent).toBe('组件')
  // 往返：组件 → 元素（可逆）
  await hz.update(h(Comp, {}), h('div', { class: 'el2' }, '元素2'))
  expect(hz.root.querySelector('.el2'), '组件 → 元素（往返可逆）').toBeTruthy()
  expect(hz.root.querySelector('.comp')).toBe(null)
})

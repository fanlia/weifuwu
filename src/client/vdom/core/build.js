...
import { renderComponent, createComponentRegistry } from './node/component.ts';
/**
 * 创建渲染分发器（**共享——build 首帧/diff 新侧渲染同一实现**）：
 * emit(v, parent, index, ref)——kindOf 分类 → 各 node/ 文件渲染
 */
export function createRenderDispatcher(emitCommand, ctx, registry) {
    const emit = async (v, parent, index, ref) => {
        ...
        switch (kindOf(v)) {
            ...
            case 'array': {
                for (const [i, c] of v.entries())
                    await emit(c, parent, index + i, ref);
                return;
            }
            ...
            case 'element': {
                await renderNative(v, id, parent, ref, emitCommand, emit);
                return;
            }
 ...
 */
export function renderToStream(root, ctx, registry) {
    const sharedCtx = (ctx ?? {});

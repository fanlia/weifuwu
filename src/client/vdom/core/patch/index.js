...
export class CommandApplier {
    ...
    /** portal 容器（key → 容器元素） */
    portalContainers = new Map();
    container;
    doc;
    registry;
    constructor(container, doc, registry) {
    ...
    remapSubtree(oldPrefix, newPrefix) {
        const remap = (map, key) => {
            const v = map.get(key);
            if (v === undefined)
                return;
        ...
        for (const id of [...this.nodes.keys()]) {
            if (id === oldPrefix || id.startsWith(oldPrefix + '.')) {
                ...
                if (el && el.nodeType === 1)
                    el.setAttribute('data-wf-id', newPrefix + id.slice(oldPrefix.length));
            }
        }
    ...
    parentOf(cmd) {
        ...
        return this.nodes.get(cmd.parent) ?? null;
    }

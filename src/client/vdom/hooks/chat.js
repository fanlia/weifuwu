 ...
 */
let _idSeq = 0;
const newId = () => `m${Date.now().toString(36)}${(_idSeq++).toString(36)}`;
/** useChat（工厂调用——会话生命周期——订阅经 useExternal） */
export function useChat(env, opts) {
    const idx = env.nextHookIndex();
    const state = env.getHookState(idx) ?? {
        messages: [],
        status: 'idle',
        subs: new Set(),
        controller: null,
    ...
    if (state.messages.length === 0) {
        const init = [];
        if (opts.system)
            init.push({ id: newId(), role: 'system', content: opts.system });
        if (opts.initialMessages)
            init.push(...opts.initialMessages);
        state.messages = init;
    }
    const notify = () => {
    ...
    /** 流式行解析（默认：{ content } 累积） */
    const parseChunk = opts.parseChunk ?? ((line) => {
    ...
    const handle = {
        ...
        async send(text) {
            ...
            const assistant = { id: newId(), role: 'assistant', content: '', status: 'streaming' };
            ...
            try {
                ...
                if (!res.ok || !res.body)
                    throw new Error(`[vdom] chat 请求失败 ${res.status}`);
        ...
        retry() {
            ...
            if (lastUser)
                void handle.send(lastUser.content);
        },
        reset() {
            state.messages = [];
            state.status = 'idle';
        ...
        async approve(decision, note, modifiedArgs) {
            // 决策应用到最后一个未审批工具调用（HITL——decision/note/modifiedArgs）
            let applied = false;
            state.messages = state.messages.map((m) => ({
                ...m,
                toolCalls: m.toolCalls?.map((tc) => {
                    if (applied || tc.approved)
                        return tc;
                    applied = true;
                    return {
                        ...tc,

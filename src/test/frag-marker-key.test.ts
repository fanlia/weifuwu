...
describe('数组项边界标记（fragment-start/end + key/id）', () => {
  it('嵌套数组项标记带 key（外层下标）；无 id 省略', async () => {
    ...
    assert.ok(comments[0]?.includes('type=fragment-start key=0'), `start 标记统一格式+key: ${comments[0]}`)
    assert.ok(comments[1]?.includes('type=fragment-end key=0'), `end 标记统一格式+key: ${comments[1]}`)
  ...
  it('多层嵌套：内层数组项标记 key 层级独立（各自内层下标）', async () => {
    ...
    const w = container.querySelector('.w')!
    const comments = [...w.childNodes].filter(n => n.nodeType === 8).map(n => n.nodeValue)
    const starts = comments.filter(c => c?.includes('type=fragment-start'))

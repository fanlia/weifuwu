...
test('UI-5：import 边界——components/hooks/middleware/services 零 import engines/', () => {
  const areas = ['src/components', 'src/ui-dom/hooks', 'src/ui-dom/middleware', 'src/ui-dom/services', 'src/ui-dom/contracts']
...
test('UI-5：引擎注册单一入口——services/render-service 的 setRenderer', () => {
  const src = readFileSync(join(root, 'src/ui-dom/services/render-service.ts'), 'utf8')
  assert.ok(src.includes('export function setRenderer'), 'setRenderer 导出（引擎注册单一入口）')
  const index = readFileSync(join(root, 'src/ui-dom/index.ts'), 'utf8')
  assert.ok(index.includes('setRenderer(vdom3Renderer)'), 'index.ts 门面注册当前引擎（v5 = 改这一行）')
  const adapter = readFileSync(join(root, 'src/ui-dom/engines/vdom3/adapter.ts'), 'utf8')

...
describe('react SSR', () => {
  ...
  // Helper component types for layout tests
  ...
  it('ctx.render serializes data to script tag', async () => {
    const app = new Router()
    app.use(react())
  ...
  it('layout nesting via mount — inner wraps inside outer', async () => {
    ...
    // Order check: outer (root div) wraps inner (data-layer) wraps content
  ...
  it('ctx.renderStream returns streaming HTML', async () => {
    const app = new Router()
    app.use(react())
    app.get('/', async (_req, ctx) => ctx.renderStream(h('h2', null, 'Streamed')))
  ...
  it('ctx.renderStream completes and contains expected content', async () => {
    const app = new Router()
    app.use(react())
    app.get('/', async (_req, ctx) =>
      ctx.renderStream(
        h('div', null,
          h('span', null, 'streamed content'),
        ),
      ),
    )
    ...
    assert.match(text, /<!DOCTYPE html>/)
    assert.match(text, /<span>streamed content<\/span>/)
  })
})

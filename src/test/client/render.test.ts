...
import { render, patchValue, mountVNode, buildVNode } from '../../ui-dom/render.ts'

/** 模式 A：async 组件需预构建后渲染——渲染器测试统一走此 */
async function buildRender(v: any, c: WfuiContext = ctx): Promise<Node | null> {
  await buildVNode(v, c)
  return (0, render)(v, c)
}
...
describe('render', () => {
  it('渲染原生元素', async () => {
    ...
    const el = await buildRender(v, ctx) as HTMLElement
  ...
  it('渲染组件', async () => {
    const Cmp = async (_: any) => (props: any) => jsx('p', { children: props.name })
    const el = await buildRender(jsx(Cmp, { name: 'Alice' }), ctx) as HTMLElement
    assert.equal(el.tagName, 'P')
    assert.equal(el.textContent, 'Alice')
  })

  it('渲染 Fragment', async () => {
    const v = jsx(Fragment, { children: [jsx('span', { children: 'a' }), jsx('span', { children: 'b' })] })
    const frag = await buildRender(v, ctx) as DocumentFragment
    assert.equal(frag.childNodes.length, 2)
    assert.equal(frag.childNodes[0].textContent, 'a')
  })

  it('渲染 null/undefined/boolean 返回 null', async () => {
    assert.equal(await buildRender(null, ctx), null)
    assert.equal(await buildRender(undefined, ctx), null)
    assert.equal(await buildRender(false, ctx), null)
    assert.equal(await buildRender(true, ctx), null)
  })

  it('渲染数字', async () => {
    assert.equal((await buildRender(42, ctx)).textContent, '42')
    assert.equal((await buildRender(0, ctx)).textContent, '0')
  })

  it('渲染空 children（props 为 null）', async () => {
    const el = await buildRender(jsx('div', null), ctx) as HTMLElement
    assert.equal(el.innerHTML, '')
  })

  it('渲染 style 对象', async () => {
    const el = await buildRender(jsx('div', { style: { color: 'red', fontSize: '14px' } }), ctx) as HTMLElement
    assert.equal(el.style.color, 'red')
    assert.equal(el.style.fontSize, '14px')
  })

  it('渲染 style 的 CSS 变量（--wf-cols 等必须 setProperty）', async () => {
    const el = await buildRender(jsx('div', { style: { '--wf-cols': 'repeat(3, 1fr)', padding: 8 } }), ctx) as HTMLElement
    assert.equal(el.style.getPropertyValue('--wf-cols'), 'repeat(3, 1fr)', 'CSS 变量必须生效')
    assert.equal(el.style.padding, '8px', '数字转 px 不回归')
    // 数值 CSS 变量保持字符串
    const el2 = await buildRender(jsx('div', { style: { '--wf-z': 5 } }), ctx) as HTMLElement
    assert.equal(el2.style.getPropertyValue('--wf-z'), '5')
  })

  it('patch 路径更新 CSS 变量（patchProps 与 setProp 对齐）', async () => {
  ...
  it('渲染 style 字符串作为属性', async () => {
    const el = await buildRender(jsx('div', { style: 'color:red' }), ctx) as HTMLElement
    assert.equal(el.getAttribute('style'), 'color:red')
  })

  it('绑定事件', async () => {
    let clicked = false
    const el = await buildRender(jsx('button', { onClick: () => { clicked = true } }), ctx) as HTMLElement
    el.click()
    assert.equal(clicked, true)
  })

  it('true 属性渲染为空值属性', async () => {
    const el = await buildRender(jsx('input', { disabled: true }), ctx) as HTMLElement
    assert.equal(el.getAttribute('disabled'), '')
  })

  it('false 属性不渲染', async () => {
    const el = await buildRender(jsx('input', { disabled: false }), ctx) as HTMLElement
    assert.equal(el.hasAttribute('disabled'), false)
  })

  it('aria-* boolean 必须显式 true/false 字符串（枚举语义——ReasoningBlock CDD 暴露）', async () => {
    const open = await buildRender(jsx('button', { 'aria-expanded': true }), ctx) as HTMLElement
    assert.equal(open.getAttribute('aria-expanded'), 'true')
    const closed = await buildRender(jsx('button', { 'aria-expanded': false }), ctx) as HTMLElement
    assert.equal(closed.getAttribute('aria-expanded'), 'false')
    const ariaHidden = await buildRender(jsx('div', { 'aria-hidden': true }), ctx) as HTMLElement
    assert.equal(ariaHidden.getAttribute('aria-hidden'), 'true')
  })

  it('null/undefined 属性不渲染', async () => {
    const el = await buildRender(jsx('div', { title: null, 'data-x': undefined }), ctx) as HTMLElement
    assert.equal(el.hasAttribute('title'), false)
    assert.equal(el.hasAttribute('data-x'), false)
  })

  it('className 转为 class', async () => {
    const el = await buildRender(jsx('div', { className: 'bar' }), ctx) as HTMLElement
    assert.equal(el.className, 'bar')
  })

  it('组件返回 null 返回 null', async () => {
    const NullCmp = async (_: any) => () => null
    assert.equal(await buildRender(jsx(NullCmp, {}), ctx), null)
  })

  it('组件返回非函数值返回 null', async () => {
    // 框架 catch 错误后返回 null
    const BadCmp = async () => false as any
    await assert.rejects(buildRender(jsx(BadCmp, {}), ctx), /must return a render function/)
  })

  it('直接渲染数组', async () => {
    const frag = await buildRender([jsx('i', { children: '1' }), jsx('i', { children: '2' })], ctx) as DocumentFragment
    assert.equal(frag.childNodes.length, 2)
  })

  it('vnode 设置 el 属性', async () => {
    const v = jsx('section', { children: 'x' })
    await buildRender(v, ctx)
...
describe('patchValue', () => {
  it('更新文本内容', async () => {
    ...
    await mountVNode(container, oldV, ctx)
  ...
  it('替换不同类型', async () => {
    const container = browser.createElement('div')
    await mountVNode(container, jsx('span', { children: 'text' }), ctx)
  ...
  it('新增元素（oldInput=null）', async () => {
    const container = browser.createElement('div')
    patchValue(container, null, null, jsx('p', { children: 'new' }), ctx)
    assert.equal(container.firstChild?.textContent, 'new')
  })

  it('新增元素（oldNode 有 parentNode 时插入前面）', async () => {
    const container = browser.createElement('div')
    await mountVNode(container, jsx('span', { children: 'existing' }), ctx)
  ...
  it('newInput 和 oldInput 都为 null 返回 null', async () => {
    assert.equal(patchValue(browser.createElement('div'), null, null, null, ctx), null)
  })

  it('从文本更新为文本', async () => {
    const container = browser.createElement('div')
    await mountVNode(container, jsx('p', { children: 'old' }), ctx)
  ...
  it('从元素替换为文本', async () => {
    const container = browser.createElement('div')
    await mountVNode(container, jsx('span', { children: 'x' }), ctx)
...
describe('component patchValue', () => {
  it('组件重渲染保持状态', async () => {
    const container = browser.createElement('div')
    let count = 0
    const Cmp = async (_: any, c: WfuiContext) => {
      const $ = c.ui!.$
  ...
  it('组件重渲染更新 props', async () => {
    const container = browser.createElement('div')
    const Cmp = async (_props: any, c: WfuiContext) => {
      const $ = c.ui!.$
      return (props: any) => {
        $.sum = ($.sum || 0) + props.x
  ...
  it('组件第一次返回 null，第二次返回元素', async () => {
    const container = browser.createElement('div')
    let first = true
    const Cmp = async (_: any) => () => first ? null : jsx('span', { children: 'ok' })
    const v1 = jsx(Cmp, {})
    await mountVNode(container, v1, ctx)
...
describe('keyed children diff', () => {
  it('按 key 重新排序', async () => {
    const container = browser.createElement('div')
    const Cmp = async (_: any) => (props: any) => jsx('div', { children: props.label })
    const oldV = jsx('div', { children: [
      jsx(Cmp, { label: 'A' }, 'a'),
      jsx(Cmp, { label: 'B' }, 'b'),
      jsx(Cmp, { label: 'C' }, 'c'),
  ...
  it('同 key 节点类型变化时 insertBefore 不失效', async () => {
  ...
  it('组件输出从元素变为 null 再恢复', async () => {
    // Drawer open/close 模式：open→render div→close→null→open→div
    const container = browser.createElement('div')

    let show = true
    const Comp = async (_: any, _ctx: any) => {
      return () => show
  ...
  it('patchValue 对组件输出 null 返回 null 不使 insertBefore 失效', async () => {
    // 验证 patchValue 对组件输出 null 时返回 null 的行为
    // 不导致 patchKeyedChildren 的 insertBefore 引用失效
    const container = browser.createElement('div')

    let showB = true
    const A = async (_: any, _ctx: any) => () => jsx('span', { children: 'A' }, 'a')
    const B = async (_: any, _ctx: any) => () => showB ? jsx('b', { children: 'B' }, 'b') : null
    const C = async (_: any, _ctx: any) => () => jsx('span', { children: 'C' }, 'c')
...
describe('mountVNode', () => {
  it('清空容器并挂载', async () => {
    ...
    await mountVNode(container, jsx('p', { children: 'new' }), ctx)
    assert.equal(container.innerHTML, '<p>new</p>')
  })

  it('挂载 Fragment', async () => {
    const container = browser.createElement('div')
    await mountVNode(container, jsx(Fragment, { children: [jsx('a', { children: '1' }), jsx('b', { children: '2' })] }), ctx)
    assert.equal(container.children.length, 2)
  ...
  it('挂载数组', async () => {
    const container = browser.createElement('div')
    await mountVNode(container, [jsx('span', { children: 'a' }), jsx('span', { children: 'b' })] as any, ctx)
    assert.equal(container.children.length, 2)
  })

  it('空容器', async () => {
    const container = browser.createElement('div')
    await mountVNode(container, null as any, ctx)
...
describe('edge cases', () => {
  it('patchValue 返回 oldNode 当新类型无法处理', async () => {
    const container = browser.createElement('div')
    // 先渲染一个正常文本
    container.textContent = 'old'
  ...
  it('patchValue 时 oldNode 为 null（native element）', async () => {
    const container = browser.createElement('div')
  ...
  it('normalize 处理单元素和 null', async () => {
    const container = browser.createElement('div')
    // children 为单个字符串（非数组）
  ...
  it('patchSimpleChildren 处理 oldChild 缺少 existingNode', async () => {
    const container = browser.createElement('div')
  ...
  it('map 生成的子节点数组正确渲染', async () => {
    ...
    const el = await buildRender(nav, ctx) as HTMLElement
  ...
  it('三元表达式从空状态切换到列表时正确移除旧元素', async () => {
  ...
  it('组件 VNode 删除时递归清理 _child 的 ref', async () => {
    const container = browser.createElement('div')
    let cleaned = false

    // 子组件：返回带 ref 的元素
    const Child = async (_: any) =>
      () => ({
        type: 'span' as const,
    ...
    await mountVNode(container, v, ctx)
...
describe('two-phase component model', () => {
  it('mount runs once, render runs each time', async () => {
    let mountCount = 0
    let renderCount = 0

    const Comp = async (_props: any, compCtx: any) => {
      mountCount++
      return (_props2: any) => {
        renderCount++
  ...
  it('ref called on component mount and unmount', async () => {
    let refCalls: (string | null)[] = []

    const Comp = async (_: any) =>
      () => ({
        type: 'div' as const,
  ...
  it('parent unmount calls child ref(null)', async () => {
  ...
  it('$ auto-dirty triggers re-render via patchValue', async () => {
    let renderCount = 0
    const Comp = async (_props: any, compCtx: any) => {
      const $ = compCtx.ui.$
      $.count = 0
  ...
  it('parent re-render preserves child mount state', async () => {
  ...
  it('three-state skip: DemoButton 结构，静态子组件跳过 render', async () => {
    ...
    const v = { type: Parent as any, props: {}, key: undefined }
    await mountVNode(container, v, skipCtx)
...
describe('keyed diff DOM mutations', () => {
  ...
  it('删除 1 项: 仅删除节点产生 DOM 修改', async () => {
    const container = browser.createElement('div')
    browser.bodyAppend(container)
  ...
  it('新增 + 删除混合: 1 insertBefore + 1 removeChild', async () => {
    const container = browser.createElement('div')
    browser.bodyAppend(container)
    const oldV = { type: 'div', props: { children: [span('A','a'), span('B','b'), span('C','c'), span('D','d')] }, key: undefined }
  ...
  it('Portal: null→开→关 正确清理 remoteEl', async () => {
    // 模拟 Modal: open=false → null, open=true → Portal
    let isOpen = false

    const Comp = async (_init: any, _compCtx: any) =>
      () => {
  ...
  it('SVG 元素使用 createElementNS 渲染', async () => {
    ...
    const el = await buildRender(v, ctx) as SVGElement
  ...
  it('innerHTML prop 忽略 children', async () => {
    const container = browser.createElement('div')
  ...
  it('ref 回调在 mount/unmount 时调用', async () => {
  ...
  it('组件返回 Fragment 渲染多个根节点', async () => {
...
describe('ref 替换语义（框架修复）', () => {
  ...
  it('内联 ref + null 分支清理：重渲染不触发清理（ref(null) 只在真正卸载时调用）', async () => {
    // 模拟 AiChat 的 unwatch 模式：null 分支是清理逻辑
    const ref1 = async (el: any) => { if (!el) cleanupCount++ }
    const ref2 = async (el: any) => { if (!el) cleanupCount++ }
    let cleanupCount = 0
...
it('children 中间 null 组件 diff 不错位（mapChildDomNodes _refNode 定位）', async () => {
  // 场景：children = [div, NullComp(渲染 null), div]——中间 null 组件无 DOM
  // 按位置 diff 时后续子项必须正确更新（壳内容区模式切换静默失效回归）
  const NullComp = async () => () => null
  const container = browser.createElement('div')
  // 真实场景（壳模式切换）：null 组件（closed Drawer）后是整块替换的组件区
  const PanelA = async () => () => jsx('div', { class: 'panel-a' }, null)
  const PanelB = async () => () => jsx('div', { class: 'panel-b' }, null)
  const makeV = async (panel: any) => jsx('div', { children: [
    jsx('span', { children: 'H' }),
    jsx(NullComp, {}),
    jsx(panel, {}),
  ]})
  const v1 = await makeV(PanelA)
  await mountVNode(container, v1, ctx)
  const el = container.firstChild as HTMLElement
  assert.ok(el.querySelector('.panel-a'), 'null 组件后组件区渲染（A）')

  // patch：null 组件后组件切换（Drawer closed 时切模式）——必须替换为 B
  // 模式 A：diff 的 v2 由 buildVNode 预构建（async 组件）
  const v2 = await makeV(PanelB)
  await buildVNode(v2, ctx)
  patchValue(container, el, v1, v2, ctx)

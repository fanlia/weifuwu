 ...
 * - D1 async 工厂（乱序 resolve——setTimeout delay 控制时序——确定性输出）
 * - D2 async renderFn（Promise 输出——终态等价）
 * - D3 throw 组件（错误路径——mount 失败清理 + 重试恢复——P1 修复回归）
...
// ── D1/D2：async 工厂 + async renderFn（乱序 resolve） ──────────────
// delay 固定（0/1ms——setTimeout FIFO——确定性）——乱序：A(1ms) 在 B(0ms) 后完成
let asyncSeq = 0

function makeAsyncFactory(kind: 'el' | 'hole'): () => any {
  const delay = [0, 1][asyncSeq++ % 2]
  const label = 'async' + asyncSeq
  const out = kind === 'el' ? h('div', { class: 'async-el' }, label) : null
  const f = () =>
    new Promise<() => VNode | null>((resolve) => {
      setTimeout(() => resolve(() => out as VNode), delay)
    })
  return f as any
}
...
/** 错误路径验证（相位区分——R2 语义）：
 *  mount：① build 流显式 reject（错误传播——不静默）② registry 无
 *  mounting 残留（P1 修复回归——占位清理）③ 替换正常组件 diff → 自愈。
 *  renderFn：① 流**正常完成**（R2 组件级 hole 降级——不炸整树）+
 *  组件槽位 hole 锚 ② rec **保留**（mounted——下一拍重试自愈）③ 同位置
 *  修复 diff → 终态正常。 */
async function verifyThrowContract(
  bad: VNode, good: VNode, registry: ComponentRegistry, phase: 'mount' | 'renderFn',
): Promise<string | null> {
  if (phase === 'mount') {
    try {
      await drainStream(renderToStream(bad, {}, createComponentRegistry()))
      return '① throw 组件 build 流未 reject（错误被静默）'
    } catch (e) {
      if (!String(e).includes('fuzz-')) return `① 错误类型不符: ${String(e)}`
    }
    const badIds = [...registry.keys()].filter((k) => k.startsWith('root'))
    if (badIds.length > 0) return `② registry 残留（mounting 占位未清理）: ${badIds.join(',')}`
  } else {
    const cmds = await drainStream(renderToStream(bad, {}, registry))
    if (!cmds.some((c) => c.op === 'done')) return '① renderFn 错误未降级——流未完成'
    if (!cmds.some((c) => c.op === 'createAnchor')) return '① renderFn 错误未降级 hole 锚'
    const ids = [...registry.keys()].filter((k) => k.startsWith('root'))
    if (ids.length === 0) return '② renderFn 错误后 rec 未保留（应为 mounted——下一拍重试）'
  }
  // ③ 修复自愈（同位置正常组件 diff——终态正常）
  const diff = await verifyEquivalence(good, good, registry)
...
test('D1：async 工厂（乱序 resolve）——终态等价（多种子 × 200 对）', async () => {
  const rnd = mulberry32(2027)
  let mismatches = 0
  let sample = ''
  for (let i = 0; i < 200; i++) {
    const mk = () => h('div', {}, [
      h(makeAsyncFactory('el'), {}),
      h(makeAsyncFactory('hole'), {}),
      h('span', {}, 'x' + (i % 2)),
    ]) as VNode
    const oldT = mk()
    const newT = mk()
...
test('D3：throw 组件——错误传播 + mount 清理 + 修复自愈（2 相位 × 多种子）', async () => {
  for (const phase of ['mount', 'renderFn'] as const) {
    for (const seed of [7, 42, 99]) {
      ...
      for (let i = 0; i < 10; i++) {
        const bad = mkBad()
        // 错误路径契约（build 传播 + registry 清理 + 同位置修复自愈）
        const issue = await verifyThrowContract(bad, mkGood(), reg, phase)

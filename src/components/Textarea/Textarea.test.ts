...
describe('Textarea', () => {
  it('renders a textarea element', () => {
    const vnode = renderVNode(Textarea, {}, createTestCtx())!
    const ta = childrenOf(vnode).find((c: any) => c?.type === 'textarea')
    assert.ok(ta, 'should have a textarea element')
  })

  it('renders label when provided', () => {
    const vnode = renderVNode(Textarea, { label: '简介' }, createTestCtx())!
  ...
  it('accepts custom rows', () => {
    const vnode = renderVNode(Textarea, { rows: 6 }, createTestCtx())!
    const ta = childrenOf(vnode).find((c: any) => c?.type === 'textarea')
    assert.equal(ta.props.rows, 6)
  })

  it('shows error message', () => {
    const vnode = renderVNode(Textarea, { error: '必填' }, createTestCtx())!
  ...
  it('forwards maxLength to the textarea', () => {
    const vnode = renderVNode(Textarea, { maxLength: 120 }, createTestCtx())!
    const ta = childrenOf(vnode).find((c: any) => c?.type === 'textarea')
    assert.equal(ta.props.maxLength, 120)
  })

  it('shows counter with value length and max', () => {
    const vnode = renderVNode(Textarea, { value: 'hello', maxLength: 10, showCount: true }, createTestCtx())!
  ...
  it('marks counter over limit', () => {
    const vnode = renderVNode(Textarea, { value: '超出长度', maxLength: 2, showCount: true }, createTestCtx())!

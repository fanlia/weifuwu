...
describe('AlertGroup', () => {
  test('合并阈值：≥3 条折叠为 +N', () => {
    const v = renderVNode(AlertGroup, { items }, createTestCtx())
  ...
  test('少于 3 条 → 平铺', () => {
    const v = renderVNode(AlertGroup, { items: items.slice(0, 2) }, createTestCtx())
    const summary = findVNode(v, (x: any) => x.props?.class?.includes('wf-alertgroup-summary'))
...
test('onClose 回调带 id（关闭按钮）', () => {
  ...
  const vnode = renderVNode(AlertGroup, { items, onClose: (id: string) => { closed = id } }, createTestCtx())!
...
test('无 onClose 不渲染关闭按钮', () => {
  ...
  const vnode = renderVNode(AlertGroup, { items }, createTestCtx())!

...
describe('ReasoningBlock', () => {
  it('默认折叠：aria-expanded=false + 正文不可见', () => {
    const v = renderVNode(ReasoningBlock, { content: '先分析用户意图…' }, createTestCtx())!
  ...
  it('点击展开 → aria-expanded=true + 正文可见', () => {
    const render = mountComponent(ReasoningBlock, { content: '推理文本' }, createTestCtx())
    let v = render()
    const toggle = findByClass(v, 'wf-reasoning-toggle')[0]
  ...
  it('defaultExpanded → 初始展开', () => {
    const v = renderVNode(ReasoningBlock, { content: 'x', defaultExpanded: true }, createTestCtx())!
    assert.equal(findByClass(v, 'wf-reasoning-toggle')[0].props['aria-expanded'], true)
  })

  it('streaming → 流式类 + 指示点', () => {
    const v = renderVNode(ReasoningBlock, { content: 'x', streaming: true }, createTestCtx())!
    assert.ok(findByClass(v, 'wf-reasoning--streaming').length === 1, '流式类')
  })

  it('自定义 label + 键盘可达（Enter 切换）', () => {
    const render = mountComponent(ReasoningBlock, { content: 'x', label: '思考过程' }, createTestCtx())
    let v = render()
    const toggle = findByClass(v, 'wf-reasoning-toggle')[0]
  ...
  it('空格键同样切换（键盘红线：可聚焦即可操作）', () => {
    const render = mountComponent(ReasoningBlock, { content: 'x' }, createTestCtx())
    let v = render()
  ...
  it('再次点击收起 → aria-expanded 还原 false', () => {
    const render = mountComponent(ReasoningBlock, { content: 'x', defaultExpanded: true }, createTestCtx())
    let v = render()
  ...
  it('streaming 时头部 label 带省略号（“已思考…”）', () => {
    const v = renderVNode(ReasoningBlock, { content: 'x', streaming: true }, createTestCtx())!

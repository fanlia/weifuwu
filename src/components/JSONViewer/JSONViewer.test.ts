...
describe('JSONViewer', () => {
  it('渲染标量键值（类型色 class）', () => {
    const vnode = renderVNode(JSONViewer, { data: { name: 'weifuwu', count: 3, active: true, note: null } }, createTestCtx())!
  ...
  it('嵌套对象/数组：depth <= defaultExpandDepth 展开', () => {
    ...
    const vnode = renderVNode(JSONViewer, { data }, createTestCtx())!
    const texts = collectText(vnode)
  ...
  it('深层嵌套（depth > defaultExpandDepth）折叠为摘要', () => {
    ...
    const vnode = renderVNode(JSONViewer, { data, defaultExpandDepth: 2 }, createTestCtx())!
    const texts = collectText(vnode)
  ...
  it('点击折叠节点展开', () => {
    ...
    const vnode = renderVNode(JSONViewer, { data, defaultExpandDepth: 1 }, createTestCtx())!
  ...
  it('顶层大量键懒展开（> maxKeys 显示 +N 提示）', () => {
    ...
    const vnode = renderVNode(JSONViewer, { data: big }, createTestCtx())!
    const texts = collectText(vnode)
  ...
  it('复制路径按钮存在', () => {
    const vnode = renderVNode(JSONViewer, { data: { a: { b: 1 } } }, createTestCtx())!
    const s = JSON.stringify(vnode)
...
it('数组折叠摘要 Array(N) + 复制按钮 aria-label 含路径', () => {
  ...
  const vnode = renderVNode(JSONViewer, { data, defaultExpandDepth: 0 }, createTestCtx())!
  const texts = collectText(vnode)

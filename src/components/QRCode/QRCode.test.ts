...
describe('QRCode', () => {
  it('renders svg', () => {
    const vnode = renderVNode(QRCode, { value: 'https://weifuwu.dev' }, createTestCtx())!
    assert.equal(vnode.type, 'svg')
  })

  it('viewBox includes quiet zone', () => {
    const vnode = renderVNode(QRCode, { value: 'hi' }, createTestCtx())!
  ...
  it('size prop sets rendered dimensions', () => {
    const vnode = renderVNode(QRCode, { value: 'hi', size: 200 }, createTestCtx())!
    assert.equal(vnode.props.width, 200)
    assert.equal(vnode.props.height, 200)
  })

  it('color and bgColor applied', () => {
    const vnode = renderVNode(QRCode, { value: 'hi', color: '#ff0000', bgColor: '#ffffff' }, createTestCtx())!
    const rects = vnode.props.children.filter((c: any) => c?.type === 'rect')
  ...
  it('different values produce different module counts', () => {
    const v1 = renderVNode(QRCode, { value: 'a' }, createTestCtx())!
    const v2 = renderVNode(QRCode, { value: 'longer-value-12345' }, createTestCtx())!

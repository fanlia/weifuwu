...
describe('InputNumber', () => {
  it('渲染输入框 + 增减按钮', () => {
    const vnode = renderVNode(InputNumber, { value: 5 }, createTestCtx())!
  ...
  it('点击 + → onChange(n+step)', () => {
    ...
    const vnode = renderVNode(InputNumber, { value: 5, onChange: (n: number | null) => { val = n } }, createTestCtx())!
  ...
  it('点击 - → onChange(n-step)，不低于 min', () => {
    let val: number | null = 0
    const vnode = renderVNode(InputNumber, { value: 2, min: 1, onChange: (n: number | null) => { val = n } }, createTestCtx())!
  ...
  it('输入数字 → onChange', () => {
    ...
    const vnode = renderVNode(InputNumber, { value: 1, onChange: (n: number | null) => { val = n } }, createTestCtx())!
    const input = findInput(vnode)
  ...
  it('输入非法字符 → 忽略（保持原值）', () => {
    ...
    const vnode = renderVNode(InputNumber, { value: 3, onChange: (n: number | null) => { val = n } }, createTestCtx())!
  ...
  it('输入 999 超 max → 即时 clamp 到 10', () => {
    let val: number | null = 0
    const vnode = renderVNode(InputNumber, { value: 1, min: 0, max: 10, onChange: (n: number | null) => { val = n } }, createTestCtx())!
  ...
  it('失焦安全网：受控值越界时 clamp（父组件未同步时兜底）', () => {
    ...
    const vnode = renderVNode(InputNumber, { value: 99, min: 0, max: 10, onChange: (n: number | null) => { val = n } }, createTestCtx())!
  ...
  it('disabled 时按钮不可点', () => {
    let clicked = false
    const vnode = renderVNode(InputNumber, { value: 5, disabled: true, onChange: () => { clicked = true } }, createTestCtx())!
    findBtn(vnode, '增加').props.onClick()

...
describe('Carousel', () => {
  it('renders slides container', async () => {
    ...
    const v = await render({ children: slides })
  ...
  it('next button advances', async () => {
    ...
    let v = await render({ children: slides })
    const next = v.props.children.find((c: any) => c?.props?.['aria-label'] === '下一张')
    next.props.onClick()
    v = await render({ children: slides })
  ...
  it('loop wraps around', async () => {
    ...
    let v = await render({ children: slides, loop: true })
    // 从 0 → 2 → 再 next 回到 0
    const dots = v.props.children.find((c: any) => c?.props?.class?.includes('wf-carousel-dots'))
    dots.props.children[2].props.onClick()
    v = await render({ children: slides, loop: true })
    const next = v.props.children.find((c: any) => c?.props?.['aria-label'] === '下一张')
    next.props.onClick()
    v = await render({ children: slides, loop: true })
    assert.match(trackOf(v).props.style.transform, /0%/)
  ...
  it('no dots when showDots=false', async () => {
    ...
    const v = await render({ children: slides, showDots: false })
  ...
  it('no arrows when showArrows=false', async () => {
    ...
    const v = await render({ children: slides, showArrows: false })
  ...
  it('autoplay advances automatically at interval', async () => {
    ...
    let v = await render({ children: slides, autoplay: true, interval: 40 })
    assert.match(trackOf(v).props.style.transform, /0%/)
    ...
    v = await render({ children: slides, autoplay: true, interval: 40 })

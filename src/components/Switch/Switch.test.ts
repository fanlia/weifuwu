...
describe('Switch', () => {
  it('renders a switch input', () => {
    const vnode = renderVNode(Switch, {}, createTestCtx())!
  ...
  it('renders label when provided', () => {
    const vnode = renderVNode(Switch, { label: '启用' }, createTestCtx())!
    const labelEl = vnode.props.children[2]
    assert.equal(labelEl.props.children, '启用')
  })

  it('sets checked state', () => {
    const vnode = renderVNode(Switch, { checked: true }, createTestCtx())!
    const input = vnode.props.children[0]
    assert.equal(input.props.checked, true)
  })

  it('sets disabled state', () => {
    const vnode = renderVNode(Switch, { disabled: true }, createTestCtx())!
    const input = vnode.props.children[0]
  ...
  it('calls onChange when clicked', () => {
    let val = false
    const vnode = renderVNode(Switch, { onChange: (v: boolean) => { val = v } }, createTestCtx())!
    const input = vnode.props.children[0]
...
it('onChange 携带勾选值（事件 target.checked）', () => {
  ...
  const vnode = renderVNode(Switch, { checked: false, onChange: (v: boolean) => { got = v } }, createTestCtx())!

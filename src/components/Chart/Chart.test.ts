...
function renderChart(props: any, ctx: WfuiContext) {
  const result = Chart(props, ctx)
...
describe('Chart', () => {
  it('renders SVG for line chart', () => {
    const vnode = renderChart({ type: 'line', data }, mockCtx())!
  ...
  it('renders SVG for bar chart', () => {
    const vnode = renderChart({ type: 'bar', data }, mockCtx())!
    const svg = vnode.props.children.find((c: any) => c?.type === 'svg')
    assert.ok(svg, 'should render svg')
  })

  it('renders SVG for pie chart', () => {
    const vnode = renderChart({ type: 'pie', data }, mockCtx())!
    const svg = vnode.props.children.find((c: any) => c?.type === 'svg')
    assert.ok(svg, 'should render svg')
  })

  it('renders title when provided', () => {
    const vnode = renderChart({ type: 'line', data, title: '月收入' }, mockCtx())!
  ...
  it('defaults to line chart', () => {
    const vnode = renderChart({ data }, mockCtx())!
    const svg = vnode.props.children.find((c: any) => c?.type === 'svg')

...
describe('Chart', () => {
  it('renders SVG for line chart', () => {
    const vnode = renderrenderVNode(Chart, { type: 'line', data }, createTestCtx())!
  ...
  it('renders SVG for bar chart', () => {
    const vnode = renderrenderVNode(Chart, { type: 'bar', data }, createTestCtx())!
    const svg = vnode.props.children.find((c: any) => c?.type === 'svg')
    assert.ok(svg, 'should render svg')
  })

  it('renders SVG for pie chart', () => {
    const vnode = renderrenderVNode(Chart, { type: 'pie', data }, createTestCtx())!
    const svg = vnode.props.children.find((c: any) => c?.type === 'svg')
    assert.ok(svg, 'should render svg')
  })

  it('renders title when provided', () => {
    const vnode = renderrenderVNode(Chart, { type: 'line', data, title: '月收入' }, createTestCtx())!
  ...
  it('defaults to line chart', () => {
    const vnode = renderrenderVNode(Chart, { data }, createTestCtx())!
    const svg = vnode.props.children.find((c: any) => c?.type === 'svg')

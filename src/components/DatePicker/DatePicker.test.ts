...
describe('DatePicker', () => {
  it('renders input with placeholder', () => {
    const vnode = DatePicker({ placeholder: '选择日期' }, mockCtx())!
  ...
  it('applies disabled class', () => {
    const vnode = DatePicker({ disabled: true }, mockCtx())!
  ...
  it('mode=date shows calendar panel via portal when open', () => {
    const ctx = mockCtx()
    ctx.ui.ready = true
    ctx.ui.$.show = true; ctx.ui.$.viewYear = 2025; ctx.ui.$.viewMonth = 6; ctx.ui.$.hour = 12; ctx.ui.$.minute = 0
    const vnode = DatePicker({ mode: 'date' }, ctx)!
  ...
  it('mode=time shows time picker', () => {
    ...
    ctx.ui.ready = true
    ctx.ui.$.show = true; ctx.ui.$.viewYear = 2025; ctx.ui.$.viewMonth = 6; ctx.ui.$.hour = 12; ctx.ui.$.minute = 0
    const vnode = DatePicker({ mode: 'time' }, ctx)!
    const portal = vnode.props.children[1]
  ...
  it('mode=range shows dual month panels', () => {
    ...
    ctx.ui.ready = true
    ctx.ui.$.show = true; ctx.ui.$.viewYear = 2025; ctx.ui.$.viewMonth = 6; ctx.ui.$.hour = 12; ctx.ui.$.minute = 0
    const vnode = DatePicker({ mode: 'range' }, ctx)!
    const portal = vnode.props.children[1]
    ...
    assert.equal(rangeWrap?.props?.children?.length, 2) // 双月
  })
  ...
  it('does not show panel when closed', () => {
    ...
    ctx.ui.$.show = false
    const vnode = DatePicker({ mode: 'date' }, ctx)!
    assert.equal(vnode.props.children.length, 1) // only input
  })
  ...
  it('calls onChange on date select', () => {
    ...
    ctx.ui.ready = true
    ctx.ui.$.show = true
    ctx.ui.$.viewYear = 2025; ctx.ui.$.viewMonth = 6; ctx.ui.$.hour = 12; ctx.ui.$.minute = 0
    const vnode = DatePicker({ mode: 'date', onChange: (v: string) => { val = v } }, ctx)!
    const portal = vnode.props.children[1]
    const panel = portal?.props?.children?.find((c: any) => c?.props?.class === 'wf-datepicker-dropdown')
    // panel.props.children = [calendarPanel]
    const calPanel = panel?.props?.children?.[0]
    // calPanel.children = [header, weekdayRow, ...gridRows]

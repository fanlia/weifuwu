...
describe('EmptyState', () => {
  it('renders container', () => {
    const vnode = renderVNode(EmptyState, {}, createTestCtx())!
  ...
  it('renders custom icon and text', () => {
    const vnode = renderVNode(EmptyState, { icon: h(Icon, { name: 'user' }), text: '没有用户' }, createTestCtx())!
    const icon = vnode.props.children[0]
    const text = vnode.props.children[1]
  ...
  it('renders hint when provided', () => {
    const vnode = renderVNode(EmptyState, { hint: '创建一个新用户' }, createTestCtx())!
  ...
  it('renders action children', () => {
    const action = '按钮'
    const vnode = renderVNode(EmptyState, { children: action }, createTestCtx())!

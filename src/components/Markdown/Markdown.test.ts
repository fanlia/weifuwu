...
describe('Markdown 组件', () => {
  ...
  it('渲染段落 + 行内粗体', () => {
    const vnode = renderVNode(Markdown, { content: '**加粗** 内容' }, createTestCtx())!
  ...
  it('链接带安全属性（noopener + target=_blank）', () => {
    const vnode = renderVNode(Markdown, { content: '[链接](https://x.dev)' }, createTestCtx())!
  ...
  it('javascript: 链接降级为纯文本（无 <a>）', () => {
    const vnode = renderVNode(Markdown, { content: '[x](javascript:alert(1))' }, createTestCtx())!
    const p = blocks(vnode)[0]
  ...
  it('空内容返回 null', () => {
    assert.equal(renderVNode(Markdown, { content: '' }, createTestCtx()), null)
  })

  it('XSS: script 内容渲染为文本节点（无元素）', () => {
    const vnode = renderVNode(Markdown, { content: '<script>alert(1)</script>' }, createTestCtx())!
    const p = blocks(vnode)[0]
...
describe('Markdown GFM', () => {
  it('删除线 ~~text~~ → <del>', () => {
    const v = renderVNode(Markdown, { content: '~~删除~~ 这段' }, createTestCtx())!
  ...
  it('表格 → thead/tbody + 对齐', () => {
    const md = '| 名称 | 数量 |\n| :--- | ---: |\n| 苹果 | 3 |\n| 香蕉 | 12 |'
    const v = renderVNode(Markdown, { content: md }, createTestCtx())!
    const s = JSON.stringify(v)
  ...
  it('表格分隔行缺失时不误判（普通段落）', () => {
    const v = renderVNode(Markdown, { content: '| 不是表格' }, createTestCtx())!
    // 无分隔行 → 走段落

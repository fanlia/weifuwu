...
describe('FileUpload', () => {
  it('renders drop zone with default text', () => {
    const vnode = renderVNode(FileUpload, {}, createTestCtx())!
  ...
  it('renders custom children instead of default', () => {
    const vnode = renderVNode(FileUpload, { children: '自定义区域' }, createTestCtx())!
    const dropZone = vnode.props.children[1]
    assert.equal(dropZone.props.children, '自定义区域')
  })

  it('renders disabled class when disabled', () => {
    const vnode = renderVNode(FileUpload, { disabled: true }, createTestCtx())!
    const dropZone = vnode.props.children[1]
    assert.match(dropZone.props.class, /wf-upload-zone--disabled/)
  })

  it('renders error class and message when error', () => {
    const vnode = renderVNode(FileUpload, { error: '文件太大' }, createTestCtx())!
  ...
  it('renders file list when value provided', () => {
    ...
    const vnode = renderVNode(FileUpload, { value: [file] }, createTestCtx())!
  ...
  it('renders hint text when provided', () => {
    const vnode = renderVNode(FileUpload, { hint: '支持图片格式' }, createTestCtx())!
    const children = vnode.props.children
  ...
  it('renders accept and maxSize hints', () => {
    const vnode = renderVNode(FileUpload, { accept: 'image/*', maxSize: 1024 * 1024 }, createTestCtx())!
    const dropZone = vnode.props.children[1]
...
describe('FileUpload 增强', () => {
  it('图片文件渲染缩略图（img 预览）', () => {
    ...
    const vnode = renderVNode(FileUpload, { value: [img, txt] }, createTestCtx())!
  ...
  it('受控无 onChange 时 warn（一次性）', () => {
    ...
    try {
      ...
      renderVNode(FileUpload, { value: [file] }, createTestCtx())
      renderVNode(FileUpload, { value: [file] }, createTestCtx())
  ...
  it('受控配 onChange 不 warn', () => {
    ...
    try {
      const file = new File(['t'], 'a.txt', { type: 'text/plain' })
      renderVNode(FileUpload, { value: [file], onChange: () => {} }, createTestCtx())
      assert.equal(warns.filter(w => w.includes('onChange')).length, 0)
    } finally { console.warn = ow }
  })

  it('uploading/progress 渲染进度条', () => {
    const vnode = renderVNode(FileUpload, { uploading: true, progress: 42 }, createTestCtx())!
  ...
  it('删除项 → onChange 回传过滤列表', () => {
    ...
    const vnode = renderVNode(FileUpload, { value: [f1, f2], onChange: (fs: File[]) => { got = fs } }, createTestCtx())!

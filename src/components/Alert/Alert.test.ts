...
it('closable + onClose：关闭按钮点击回调', () => {
  let closed = 0
  const vnode = renderVNode(Alert, { closable: true, onClose: () => closed++, children: 'x' }, mockCtx())!
...
it('variant 着色类（success/error）', () => {
  const v1 = renderVNode(Alert, { variant: 'success', children: 'x' }, mockCtx())!
  assert.ok(JSON.stringify(v1).includes('success'))
  const v2 = renderVNode(Alert, { variant: 'error', children: 'x' }, mockCtx())!
  assert.ok(JSON.stringify(v2).includes('error'))
})

it('非 closable 无关闭按钮（边界）', () => {
  const vnode = renderVNode(Alert, { children: 'x' }, mockCtx())!
  const s = JSON.stringify(vnode)

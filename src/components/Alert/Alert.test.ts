...
describe('Alert', () => {
  it('renders message', async () => {
    const vnode = await renderVNode(Alert, { children: '操作成功' }, createTestCtx())!
  ...
  it('returns null when no children', async () => {
    const result = await renderVNode(Alert, {}, createTestCtx())
    assert.equal(result, null)
  })

  it('renders all variants', async () => {
    for (const v of ['info', 'success', 'warning', 'error'] as const) {
      const vnode = await renderVNode(Alert, { variant: v, children: '消息' }, createTestCtx())!
      assert.match(vnode.props.class, new RegExp(`wf-alert--${v}`))
    }
  })

  it('renders close button when closable', async () => {
    const vnode = await renderVNode(Alert, { closable: true, children: '消息' }, createTestCtx())!
  ...
  it('calls onClose when close button clicked', async () => {
    let closed = false
    const vnode = await renderVNode(Alert, { closable: true, onClose: () => { closed = true }, children: '消息' }, createTestCtx())!
    vnode.props.children[2].props.onClick()
    assert.equal(closed, true)
  })
})

it('closable + onClose：关闭按钮点击回调', async () => {
  let closed = 0
  const vnode = await renderVNode(Alert, { closable: true, onClose: () => closed++, children: 'x' }, createTestCtx())!
...
it('variant 着色类（success/error）', async () => {
  const v1 = await renderVNode(Alert, { variant: 'success', children: 'x' }, createTestCtx())!
  assert.ok(JSON.stringify(v1).includes('success'))
  const v2 = await renderVNode(Alert, { variant: 'error', children: 'x' }, createTestCtx())!
  assert.ok(JSON.stringify(v2).includes('error'))
})

it('非 closable 无关闭按钮（边界）', async () => {
  const vnode = await renderVNode(Alert, { children: 'x' }, createTestCtx())!
  const s = JSON.stringify(vnode)

...
describe('Select 增强（键盘导航 + multiple）', () => {
  ...
  async function openSelect(props: any){
    const ctx = mockCtx()
  ...
  it('multiple: renders selected tags in trigger', async () => {
    const { vnode2 } = await openSelect({ multiple: true, value: ['a', 'c'] })
    const tags = allNodes(vnode2).filter((n: any) => n.props?.class === 'wf-select-tag')
    assert.equal(tags.length, 2)
    assert.equal(tags[0].props.children[0], '选项A')
  })

  it('multiple: click option toggles value', async () => {
    let got: any = null
    const ctx = mockCtx()
  ...
  it('multiple: tag close removes value', async () => {
    let got: any = null
    const ctx = mockCtx()
  ...
  it('keyboard: ArrowDown highlights first option, Enter selects', async () => {
    let got: any = null
    const ctx = mockCtx()
  ...
  it('keyboard: ArrowDown twice then Enter selects second option', async () => {
    let got: any = null
    const ctx = mockCtx()
    const render = await Select({ searchable: true, options: opts, onChange: (v: any) => { got = v } }, ctx) as any
  ...
  it('keyboard: highlight class applied to highlighted option', async () => {
    const ctx = mockCtx()
    const render = await Select({ searchable: true, options: opts }, ctx) as any

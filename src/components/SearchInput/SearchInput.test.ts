...
describe('SearchInput', () => {
  it('renders search container', () => {
    const vnode = renderVNode(SearchInput, {}, createTestCtx())!
  ...
  it('renders custom placeholder', () => {
    const vnode = renderVNode(SearchInput, { placeholder: '查找...' }, createTestCtx())!
    const input = vnode.props.children[1]
    assert.equal(input.props.placeholder, '查找...')
  })

  it('renders clear button when value and onClear provided', () => {
    const vnode = renderVNode(SearchInput, { value: 'test', onClear: () => {} }, createTestCtx())!
  ...
  it('does not render clear button when value is empty', () => {
    const vnode = renderVNode(SearchInput, { value: '', onClear: () => {} }, createTestCtx())!
...
it('有值时渲染清除按钮，点击触发 onClear', () => {
  let cleared = 0
  const vnode = renderVNode(SearchInput, { value: 'abc', onClear: () => cleared++ }, createTestCtx())!
...
it('空值不渲染清除按钮（边界）', () => {
  const vnode = renderVNode(SearchInput, { value: '' }, createTestCtx())!

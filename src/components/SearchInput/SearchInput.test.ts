...
describe('SearchInput', () => {
  it('renders search container', async () => {
    const vnode = await renderVNode(SearchInput, {}, createTestCtx())!
  ...
  it('renders custom placeholder', async () => {
    const vnode = await renderVNode(SearchInput, { placeholder: '查找...' }, createTestCtx())!
    const input = vnode.props.children[1]
    assert.equal(input.props.placeholder, '查找...')
  })

  it('renders clear button when value and onClear provided', async () => {
    const vnode = await renderVNode(SearchInput, { value: 'test', onClear: () => {} }, createTestCtx())!
  ...
  it('does not render clear button when value is empty', async () => {
    const vnode = await renderVNode(SearchInput, { value: '', onClear: () => {} }, createTestCtx())!
    assert.equal(vnode.props.children.length, 2) // only icon + input
  })
})

it('有值时渲染清除按钮，点击触发 onClear', async () => {
  let cleared = 0
  const vnode = await renderVNode(SearchInput, { value: 'abc', onClear: () => cleared++ }, createTestCtx())!

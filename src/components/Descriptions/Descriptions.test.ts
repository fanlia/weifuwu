...
describe('Descriptions', () => {
  it('渲染 dl 结构（语义）', () => {
    const vnode = renderVNode(Descriptions, { items }, mockCtx())!
  ...
  it('column 控制栅格列数类', () => {
    const vnode = renderVNode(Descriptions, { items, column: 2 }, mockCtx())!
    assert.match(vnode.props.class, /wf-descriptions--2/)
  })

  it('bordered 类', () => {
    const vnode = renderVNode(Descriptions, { items, bordered: true }, mockCtx())!
    assert.match(vnode.props.class, /wf-descriptions--bordered/)
  })

  it('span 跨列（grid-column）', () => {
    const vnode = renderVNode(Descriptions, { items: [{ label: 'a', value: '1', span: 2 }], column: 2 }, mockCtx())!
    const item = vnode.props.children[0]
    assert.match(item.props.style.gridColumn, /span 2/)
  })

  it('VNode value 支持（自定义渲染）', () => {
    const vnode = renderVNode(Descriptions, { items: [{ label: 'x', value: { tag: 'strong', props: {}, children: '加粗' } }] }, mockCtx())!

...
describe('Input', () => {
  it('renders an input element', () => {
    ...
    const vnode = renderVNode(Input, {}, createTestCtx())!
    assert.equal(vnode.type, 'input')
    assert.equal(vnode.props.type, 'text')
  })

  it('renders label when provided', () => {
    const vnode = renderVNode(Input, { label: '邮箱' }, createTestCtx())!
  ...
  it('shows required marker', () => {
    const vnode = renderVNode(Input, { label: '邮箱', required: true }, createTestCtx())!
    const label = childrenOf(vnode).find((c: any) => c?.type === 'label')
  ...
  it('shows error message', () => {
    const vnode = renderVNode(Input, { error: '必填' }, createTestCtx())!
  ...
  it('shows hint text', () => {
    const vnode = renderVNode(Input, { hint: '请输入邮箱' }, createTestCtx())!
  ...
  it('hides hint when error is present', () => {
    const vnode = renderVNode(Input, { hint: '提示', error: '错误' }, createTestCtx())!
    const hint = childrenOf(vnode).find((c: any) => c?.props?.class === 'wf-input-hint')
    assert.ok(!hint, 'should not have hint when error is present')
  })

  it('sets input type', () => {
    const vnode = renderVNode(Input, { type: 'email' }, createTestCtx())!
  ...
  it('applies borderless variant class', () => {
    const vnode = renderVNode(Input, { variant: 'borderless' }, createTestCtx())!
    assert.match(vnode.props.class, /wf-input--borderless/)
    const plain = renderVNode(Input, {}, createTestCtx())!

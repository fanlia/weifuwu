...
describe('ProgressBar', () => {
  it('renders progress bar', () => {
    const vnode = renderVNode(ProgressBar, {}, createTestCtx())!
    assert.equal(vnode.type, 'div')
    assert.match(vnode.props.class, /wf-progress/)
  })

  it('sets width based on value', () => {
    const vnode = renderVNode(ProgressBar, { value: 50 }, createTestCtx())!
    const fill = vnode.props.children[0]
    assert.equal(fill.props.style.width, '50%')
  })

  it('clamps value to 0-100', () => {
    const over = renderVNode(ProgressBar, { value: 200 }, createTestCtx())!
    const under = renderVNode(ProgressBar, { value: -10 }, createTestCtx())!
    assert.equal(over.props.children[0].props.style.width, '100%')
    assert.equal(under.props.children[0].props.style.width, '0%')
  })

  it('renders label when provided', () => {
    const vnode = renderVNode(ProgressBar, { label: '进度', value: 50 }, createTestCtx())!
  ...
  it('renders value percentage when showValue is true', () => {
    const vnode = renderVNode(ProgressBar, { value: 75, showValue: true }, createTestCtx())!
    assert.equal(vnode.props.class, 'wf-progress-wrap')
...
it('indeterminate：value 缺省 → 不确定态动画类 + 无 valuenow', () => {
  const vnode = renderVNode(ProgressBar, { label: '加载中' }, createTestCtx())!
...
it('status 变色 + size', () => {
  const v1 = renderVNode(ProgressBar, { value: 100, status: 'success' }, createTestCtx())!
  assert.ok(JSON.stringify(v1).includes('wf-progress-fill--success'), 'success 变体')
  const v2 = renderVNode(ProgressBar, { value: 50, size: 'lg' }, createTestCtx())!

...
describe('Toggle', () => {
  it('renders button with pressed state', () => {
    const vnode = renderVNode(Toggle, { pressed: true, children: 'B' }, mockCtx())!
  ...
  it('toggles on click', () => {
    ...
    const vnode = renderVNode(Toggle, { pressed: false, onPressedChange: (p: boolean) => { got = p }, children: 'B' }, mockCtx())!
    vnode.props.onClick()
  ...
  it('disabled: no click handler', () => {
    let called = false
    const vnode = renderVNode(Toggle, { pressed: false, disabled: true, onPressedChange: () => { called = true }, children: 'B' }, mockCtx())!
    assert.equal(vnode.props.onClick, undefined)
    assert.equal(called, false)
  })

  it('merges className and passes size', () => {
    const vnode = renderVNode(Toggle, { size: 'sm', children: 'B' }, mockCtx())!
...
describe('ToggleGroup', () => {
  ...
  it('renders all options', () => {
    const vnode = renderVNode(ToggleGroup, { type: 'multiple', options: opts, value: [] }, mockCtx())!
  ...
  it('single: value marks pressed option', () => {
    const vnode = renderVNode(ToggleGroup, { type: 'single', options: opts, value: 'italic' }, mockCtx())!
  ...
  it('single: click selects value', () => {
    ...
    const vnode = renderVNode(ToggleGroup, { type: 'single', options: opts, value: '', onChange: (v: any) => { got = v } }, mockCtx())!
  ...
  it('multiple: click removes selected', () => {
    let got: string[] = []
    const vnode = renderVNode(ToggleGroup, { type: 'multiple', options: opts, value: ['bold'], onChange: (v: any) => { got = v } }, mockCtx())!
  ...
  it('keyboard: ArrowRight in single mode moves selection', () => {
    let got: string | null = null
    const vnode = renderVNode(ToggleGroup, { type: 'single', options: opts, value: 'bold', onChange: (v: any) => { got = v } }, mockCtx())!
  ...
  it('disabled: no interaction', () => {
    const vnode = renderVNode(ToggleGroup, { type: 'single', options: opts, value: '', disabled: true }, mockCtx())!
    assert.equal(vnode.props.children[0].props.onClick, undefined)
  })

  it('renders sizes', () => {
    const vnode = renderVNode(ToggleGroup, { type: 'single', options: opts, value: '', size: 'lg' }, mockCtx())!

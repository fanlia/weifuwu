...
describe('Button', () => {
  it('renders primary variant by default', () => {
    const vnode = renderVNode(Button, { children: '提交' }, createTestCtx())!
    assert.equal(vnode.type, 'button')
    assert.match(vnode.props.class, /wf-btn--primary/)
  })

  it('sets type=button by default', () => {
    const vnode = renderVNode(Button, { children: '点击' }, createTestCtx())!
    assert.equal(vnode.props.type, 'button')
  })

  it('accepts type=submit', () => {
    const vnode = renderVNode(Button, { type: 'submit', children: '提交' }, createTestCtx())!
    assert.equal(vnode.props.type, 'submit')
  })

  it('disables when loading', () => {
    const vnode = renderVNode(Button, { loading: true, children: '保存' }, createTestCtx())!
    assert.equal(vnode.props.disabled, true)
  })

  it('disables when disabled prop is set', () => {
    const vnode = renderVNode(Button, { disabled: true, children: '不可用' }, createTestCtx())!
    assert.equal(vnode.props.disabled, true)
  })

  it('renders all variants', () => {
    for (const v of ['primary', 'secondary', 'ghost', 'danger'] as const) {
      const vnode = renderVNode(Button, { variant: v, children: '测试' }, createTestCtx())!
  ...
  it('renders all sizes', () => {
    for (const s of ['sm', 'md', 'lg'] as const) {
      const vnode = renderVNode(Button, { size: s, children: '测试' }, createTestCtx())!
      assert.match(vnode.props.class, new RegExp(`wf-btn--${s}`))
    }
  })

  it('applies block class', () => {
    const vnode = renderVNode(Button, { block: true, children: '全宽' }, createTestCtx())!
    assert.match(vnode.props.class, /wf-btn--block/)
  })

  it('renders children as content', () => {
    const vnode = renderVNode(Button, { children: '确定' }, createTestCtx())!
  ...
  it('透传 id（测试定位/锚点）', () => {
    const vnode = renderVNode(Button, { id: 'async-click', children: '点击' }, createTestCtx())!

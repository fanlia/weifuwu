...
describe('office/zip（自研容器——零依赖）', () => {
  test('writeZip → readZip 往返（store 无压缩 + CRC32）', () => {
    const files = new Map([
    ...
    const back2 = readZip(zip2)
    assert.equal(back2.get('测试/文件.txt')![0], 0x6f)
  })

  test('readZip：损坏输入抛错（诚实——非静默）', () => {
...
describe('office/docx（docx ↔ ODES DocState——参考 office2json 算法）', () => {
  ...
  test('往返：表格 embed（w:tbl ↔ 表格 html 快照）', () => {
    const doc: DocState = {
      text: `表前文字\n${EMBED_CHAR}\n表后\n`,
      blockProps: [],
      marks: [],
  ...
  test('往返：图片 embed（data url → media → w:drawing → data url）', () => {
    // 1x1 png
  ...
  test('往返：pre 代码块 → 每行一段（格式裁剪——文本保留）', () => {
    const doc: DocState = {
      text: `${EMBED_CHAR}\n`,
      blockProps: [],
      marks: [],
  ...
  test('导入：样式名/对齐映射 + 空段落 + 裁剪 warning', () => {
  ...
  test('导出格式：document.xml 可被自研解析器解析（OOXML 规整）', () => {
    const doc: DocState = {
      text: '第一段\n第二段\n',

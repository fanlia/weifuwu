...
import { shape } from '../../level0/db/shape.ts'
import type { GqlShapeOptions, GqlShapeOutput, RestShapeOptions, RestShapeOutput } from '../../level0/db/generator-contracts.ts'
import type { SelectBuilder, InsertBuilder, UpdateBuilder, DeleteBuilder, QueryBuilder, WhereExpr } from '../../level0/db/query.ts'
import type { Infer } from '../../level0/zod.ts'
import { ValidationError } from '../../level0/db/errors.ts'
...
export interface OrmTable<S extends ZodRawShape> {
  ...
  c: { [K in keyof S]: { ref: string; readonly __out: import('../../level0/zod.ts').Infer<S[K]> } }
...
function makeOrm(adapter: DbAdapter, tenant: OrmTenant | undefined, tables: Map<string, { t: OrmTable<ZodRawShape> }>): Orm {
  ...
  function table<S extends ZodRawShape>(name: string, shapeDef?: S): OrmTable<S> {
    ...
    const t = {
      c: c as { [K in keyof S]: { ref: string; readonly __out: import('../../level0/zod.ts').Infer<S[K]> } },

 ...
 * （默认实现 src/level5/server/ws/native/——W5 自研 RFC6455；`ws` 仅差分参考）。
 */

import type { Duplex } from 'node:stream'
import type { IncomingMessage } from 'node:http'
import type { Context, Hub, WebSocket } from '../level0/types.ts'

/** Hub 类型自 L0 types.ts 重出（历史导入路径兼容）。 */
export type { Hub } from '../level0/types.ts'
 ...
 * - 默认：自研 RFC6455（src/level5/server/ws/native/——`weifuwu` 入口 serve() 自动注入）

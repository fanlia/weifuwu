...
import { createTrie, trieRegister } from '../../../level0/router/trie.ts'
import { dispatchRouter, type RouterPipeline, type RouteMatch } from '../../../level0/router/pipeline.ts'
import type { SharedHandler } from '../../../level0/router/types.ts'
...
describe('B2 双端对账（前后端语义漂移的结构性防线）', () => {
  ...
  test('server Router 与 client UIRouter：GET 命中/params/404 分类全等', async () => {
    const { Router } = await import('../../../level2/router.ts')
    const { UIRouter } = await import('../../../level3/vdom/router.ts')

...
export const AutoComplete: Component<AutoCompleteProps> = (_init, ctx: UIContext)=> {
  ...
  const syncDropdown = (dropdown: import('../../../../level6/client/vdom/index.ts').VNode | null): void => {
    if (open && !handle)
  ...
  const pick = (option: AutoCompleteOption)=> {
    ...
    inputCtrl?.setSelectedLabel(String(option.label ?? option.value)) // 关闭后回填（label 可 VNodeChild——转字符串）
    activeIndex = -1
    setOpen(false)
  }

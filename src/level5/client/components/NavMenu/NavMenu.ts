...
import { h, createItem } from '../../../../level6/client/vdom/index.ts'
...
export const NavMenu: Component<NavMenuProps> = (_init, ctx: UIContext)=> {
  ...
  /** 顶层子菜单句柄 */
  let subHandle: import('../../../../level3/vdom/hooks/popup-manager.ts').PopupHandle | null = null
  const syncSub = (content: import('../../../../level6/client/vdom/index.ts').VNode): void => {
  ...
  let nestedHandle: import('../../../../level3/vdom/hooks/popup-manager.ts').PopupHandle | null = null
  const syncNested = (content: import('../../../../level6/client/vdom/index.ts').VNode): void => {
    ...
    items.map(item => {
      ...
      return h('div', {
        ...
        onMouseEnter: ()=> {
          ...
          if (item.disabled) return
          if (hasNested) {
            if (nestedKey !== item.key) { nestedKey = item.key; ctx.render() }
    ...
    const navVn = h('nav', {
      ...
      return h('div', navItem({ active: activeKey === item.key, open: hasChildren && isOpen }, {
        ...
        onMouseEnter: ()=> {
          ...
          if (item.disabled) return
          if (hasChildren) {
            if (openKey !== item.key) { openKey = item.key; nestedKey = null; ctx.render() }

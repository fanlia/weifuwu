 ...
 * `src/level6/apps/showcase/server.ts` 与 `src/test/scenario/server.ts` 把全部文件塞进
 ...
 * @param layoutDir `src/level5/client/layout` 绝对/相对路径
 */
export async function bundleLayout(layoutDir: string): Promise<CssBundle> {
  ...
  for (const f of files) {
    ...
    if (!layer) {
      throw new Error(`layout 文件未登记 @layer 映射: ${f}（在 src/level5/client/layout/bundle.ts LAYER_OF 中登记）`)
    }
  ...
  const css = `/* weifuwu/client/layout — CSS 布局原语 + 设计 Token（装配单源：src/level5/client/layout/bundle.ts） */\n\n@layer ${LAYER_ORDER.join(', ')};\n\n${chunks.join('\n\n')}\n`
  return { css, inputs }
}

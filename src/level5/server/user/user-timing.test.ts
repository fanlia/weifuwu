...
describe('userSystem timing 拉平（B3）', () => {
  it('不存在邮箱 / SSO 账号 / loginApp：均执行一次 verifyPassword（dummy 拉平）', async () => {
    ...
    const real = await import('./password.ts')
    ...
    const { userSystem } = await import('../../../level6/server/user/index.ts')
    const { createMemoryOrm } = await import('../../../level2/db/memory-sql.ts')
    const { WEIFUWU_USER_SCHEMA } = await import('../../../level6/server/user/index.ts')

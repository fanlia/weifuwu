...
describe('serve graceful shutdown (S2)', () => {
  ...
  it('SIGTERM drains in-flight then exits 0（滚动发布语义）', async () => {
    // 子进程 fixture：默认启用 shutdown（SIGTERM handler）——路径：core/ → ../../../level6/index.ts = src/level6/index.ts
    const entry = pathToFileURL(new URL('../../../level6/index.ts', import.meta.url).pathname).href
    const fixture = [

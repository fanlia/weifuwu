...
it('根路径 "/" 与通配 "/*" 并存（精确优先——showcase SSR 事故回归）', async () => {
  ...
  const r1 = await handler(new Request('http://x/'), { params: {} } as never)
  assert.equal(await r1.text(), 'ROOT', '根路径精确命中')
  const r2 = await handler(new Request('http://x/other'), { params: {} } as never)
  assert.equal(await r2.text(), 'WILD', '其他路径通配命中')
  const r3 = await handler(new Request('http://x/deep/path'), { params: {} } as never)

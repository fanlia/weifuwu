...
import { createMemoryOrm } from '../../../level2/db/memory-sql.ts'
import { createQueryBuilder } from '../../../level2/db/query-builder.ts'
import { compileQuery, compileSelect, rawSql } from '../../../level0/db/query.ts'
import { and, or, nowAgo } from '../../../level0/db/ops.ts'
import { ProtocolError } from '../../../level0/db/errors.ts'
import type { SelectQuery } from '../../../level0/db/query.ts'
import { z } from '../../../level0/zod.ts'

/** fixture 建表（AST 声明面——零 SQL 文本） */
function fx(mem: import('../../../level2/db/memory-sql.ts').MemorySql, name: string, columns: Record<string, unknown>, extra: Record<string, unknown> = {}): void {
...
describe('query language — E3 条件安全组合与 RawSql 面', () => {
  ...
  it('W2：where 值 undefined 显式拒绝（入口校验——compile/memory 共享 AST 源头拦截）', async () => {
    const mem = new (await import('../../../level2/db/memory-sql.ts')).MemorySql()

...
// ── ui-dom SSR 落地（定稿：SSR 是链尾落地中间件 VDOM→HTML） ──
 ...
 * 执行 router 匹配 → 中间件链 → handler（async 组件）→ vnode → renderSsr。
 *
 * ```ts
 * const { page } = await ssrPage(router, { url: '/users/42' })

...
| W2 | | **标尺档 token 化（42 形态 · 22 文件 · 49 条声明）**：`padding/margin/gap` ∈ {4,8,12,16}px → `var(--wf-space-xs/sm/(裸)/md)` + `var(--wf-gap-xs/sm/md)`（base 值全等映射——12px→`--wf-space` 裸档（space-md=16 勿混）· gap 12px→`--wf-gap-md`）。**两处修正**：① 紧凑单行格式文件（Math `padding: 0 4px` 一行双声明）行首锚点正则漏替换——改属性名白名单锚定；② **currentColor/transparent 色漏检**——发丝检测放宽为「色任意 token/关键字」（hex 锁 0 面之外——Math `border-bottom: 1px solid currentColor` · SlideCanvas `border: 1px solid transparent` 补 token 化）。**缩放实证链（Playwright 读值）**：base `--wf-space-xs`=4px/gap-sm=8px（与手写值**恒等**——showcase 328 base 视觉零变确认）· compact = **3px/6px**（**缩放生效**——原手写组件静止 4px/8px = 活体缺口修复实证）。**回归**：C1 标尺 42→**0** · 结构桶 508→506（漂移 2 项出桶随 token 化）· test:client 451 · showcase 328 · 场景 129 · dist +892B（gzip -62B——基线更新+理由）。 |
| W3 | | **半标尺补面 + 结构值定案**：① **半标尺 14 形态**（含 var 混合声明 `padding: 8px var(--wf-x)`——px∈档位即不完整标准化）→ 标尺桶扩定义（px 值域判定替代纯字面量）+ 替换 17 处 → 标尺 **0**；② **派生面豁免（L13 同款）**：含 calc/env/min/max/clamp 的声明不进任何桶——结构桶 506→479（27 项派生值（`min(var(--x,32px),calc…)` · `env(safe-area)` · `calc(50%+11px)`）移出）· 0px 零值形态豁免；③ **结构值理由定案 479 项**（组级模板：尺寸族 height/width/min-*/max-* **295 项判负 token 化**（组件域语义不同——标尺爆炸）· outline 焦点环 21 项 · box-shadow 偏移 16 · 44px 命中区 · 2px/1px 发丝级 · z-index 8（同层 DOM 顺序裁决））；④ **radius 面健康实证**（几乎全 `var(--wf-radius-*)`——手写仅 2px×2 随结构桶）。**回归**：C1 四桶全绿（0/0/479/8）· test:client 451 · showcase 328 · 场景 129 · dist +226B（gzip -19B）· tsc 0。 |

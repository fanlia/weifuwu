...
- **C ✓（收尾）**：路由层契约（Router.handler fixture——匿名 400/创建门/隔离 404/执行 201/runs/404/meta——12 测试）+ docs §7 框架系统面 + audit 七线回归门（135 页全绿）
  - 踩坑：测试级联失败（assert 中止未恢复 mock 状态——try/finally）；appIdOr 抛 Error 转 500 → HttpError(400)；runs 列表对不存在 workflow 应 404（存在性检查）

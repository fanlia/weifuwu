...
- **端点注册表**：**已判负**（W1 探针 2027-xx——26 唯一端点中 >50% 动作面·
  类型收益与 W0 重叠·双层参数样板>收益）；推翻：新端点 >30 且出现「同
  端点 3+ 消费方」实例（届时局部注册值得）
- **端点注册表（历史预判）**：待 W1 探针裁决——预判判负（URL 动态拼接 + query 参数
...
- W1（本提交）：**端点注册表判负**——探针净结果：ui/ 面 api.* 调用 32 处
  ...
  containers/{x}/{x}/campaigns/{x}/{x——**动词面**——注册表（资源 CRUD 中心）
  表达力不足）；动态拼接双层参数（{x}/{x}——id+动作枚举——注册表需 action
  键——样板>收益）；**类型收益已由 W0 路线拿到**（出参单源 = RowOf 派生——
  响应类型共享——注册表的类型面与 W0 重叠）——判负（预判兑现）

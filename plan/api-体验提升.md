...
| 波次 | 内容 | 验收 |
| --- | --- | --- |
| W0 | **错误处理链落地**：框架 `errorResponse` 语义对齐 ErrorHandler 契约（HttpError/DbError → 状态码映射——链入口统一）+ 平台 server.ts `app.onError` 接线 + agents route 试点**删 route 内 try/catch**（错误面单源——链捕获） | 契约 3（数据库错误/业务守卫/校验错误三型映射）+ 平台试点 diff（try/catch 行数下降）· 回归绿 |
...
| W4 | **docs + 回归门**：docs/server.md §5 响应/错误面增补（链落地后的 handler 最小形态）+ 全量回归门 | 五域+audit 七线 + tsc 三 0 + 平台 475 |

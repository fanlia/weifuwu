...
| 客户端/通用边界实测 | `l1/client` 71 = DOM 传递闭包 **52**（客户端运行时）+ **19**（通用运行时）——剥注释口径（见下） |
...
**闭包口径（W0 定稿——剥注释；种子+闭包规则入库，非清单硬编码）**：
种子 = 剥注释后**代码面**直接引用 DOM 全局的 **22** 文件（词表 11 项：document/window/navigator/
localStorage/sessionStorage/matchMedia/HTMLElement/MutationObserver/ResizeObserver/IntersectionObserver/
requestAnimationFrame/getComputedStyle）；**谁 import 客户端文件谁也是客户端**（传递闭包）——实测 22 → 52 / 19。
旧口径（含注释）27 → 55 / 16 废弃：注释提及 ≠ 代码使用（差集 5 文件全为注释命中）。

**level1 实测 19**：`async-guard.ts` · `dev/{effect-guard,error-counter}.ts` · `hooks/ai-stream.ts` ·
`node/{children,fragment,index,keyed,portal}.ts` · `observable/{index,observable,operators,sources,types}.ts` ·
`patch/state-machine.ts` · `ports.ts` · `store.ts` · `v2/{schedule,spy}.ts`
...
| **3** | 客户端运行时 | `l1/client` 闭包 **52**（vdom 引擎 · patch/transform/field · hooks · browser · UIContext · ssr-ui · client router） | DOM | 1.0 |
...
| `src/core/l1/client/**`（闭包外 19） | `src/level1/**` |
| `src/core/l1/server/**` | `src/level2/**` |
| `src/core/l1/client/**`（闭包内 52） | `src/level3/**` |
...
| 项 | 目标 |
| --- | --- |
| exports | **不写 `exports` 字段——`dist/` 目录即导出面**（level0..level6 子树：`weifuwu/dist/levelN/**`）；旧 22 键随字段删除自然消失；包内代码（应用）一律相对导入 |
...
| **W1 框架面迁移**（原子 commit） | ① core 116 → level0..4；② 装备 → level5（含 sandbox Go）；③ 19 入口 → level6 聚合面；④ apps 导入改相对（全库 import 一次性重写）；⑤ 旧路径与 shim 删除 + **`exports` 字段删除**（重写后零包内 specifier） | `test:levels`+`test:client`+`test:server`+`test:scenario`+`test:showcase` 绿 · `tsc` 0 · `audit:levels --check` 绿（apps 此时仍用旧 specifier → 需在 W2 前保持可运行？→ **见风险表：W1/W2 合批或 W1 内先重写 apps 相对导入**） |
...
| **W3 包面开放** | 两个 bin（应用启动器）+ files（含应用资产）+ dist 构建策略（应用编译/资产——依赖面已达标：deps = esbuild + graphql）+ 安装冒烟（**按 `weifuwu/dist/levelN/**` 路径导入**） | `npm pack` → 临时目录安装 → dist 路径导入解析 + **`npx weifuwu-showcase` 起服 curl 200** + `npx weifuwu-platform` 起服（最少依赖） |
...
- **`exports` 字段本身不写**：dist 即导出面（用户决策）——推翻条件：外部消费者需要稳定语义路径（届时可只列 levelN 子树）。
...
- `5b57f991` W0-1 去 core 更名（脚本/命令/数据/文档/契约测试）——audit:levels 三线绿 · test:levels 829+1 skip
- W0-2 分类器 v2（全库 scope）：**1273 文件分类**（L0 25 · L1 19 · L2 13 · L3 51 · L4 7 · L5 235 · L6 172 ·
  shim 116 · 测试/资产/测试辅助余量）· 未分类 0 · 三方 0 · upward 1（已知 L0→L1 类型）· viaShim 122
  （W1 重写清单）· 未解析 0 · shim 破损 0；闭包固化 **22 种子 → 52 客户端 / 19 通用**（剥注释口径——
  旧含注释口径 27/55/16 废弃，差集 5 文件全为注释命中）；红探针实证（L0 越级 import + 规模超基线 → ✖）；
  `audit:all` 十四线 exit 0（showcase :3200）

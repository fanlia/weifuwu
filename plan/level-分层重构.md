...
- `9217b1b5` W0-2 分类器 v2（143 行→全库）：1273 文件分类（L0 25 · L1 19 · L2 13 · L3 51 · L4 7 · L5 235 · L6 172 ·
  shim 116）· 未分类 0 · 三方 0 · upward 1（已知 L0→L1 类型）· viaShim 122（W1 重写清单）·
  闭包固化 22 种子 → 52 客户端 / 19 通用（剥注释口径）· 红探针实证 · audit:all 十四线 exit 0
- W0-3 迁移执行器 `scripts/level-migrate.mjs`（dry-run 默认/--apply · 解析式重写 + 单目标 shim 穿透）：

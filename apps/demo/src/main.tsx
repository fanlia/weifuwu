/**
 * weifuwu demo — 两阶段组件 + $ 响应式状态
 *
 * 核心模式：
 *   ctx.ui.$()  →  状态（深度 Proxy：$.x=val / push / [0].x=y 自动渲染）
 *   三元 + .map()  →  条件/列表
 *   mount 阶段初始化 + fetch  →  异步数据
 *   ref  →  生命周期
 */
...
function HomePage(_props: {}, ctx: WfuiContext) {
  ...
  const $ = ctx.ui.$()
  $.features = [
    { title: 'Todo', desc: 'ctx.ui.$() 响应式列表', path: '/todo' },
    { title: '表单', desc: '手动 $ 表单', path: '/forms' },
    { title: '数据', desc: 'mount 阶段 fetch', path: '/data' },
...
function DataPage(_props: {}, ctx: WfuiContext) {
  ...
  return () => (
    <div>
      ...
      <p class="text-gray-400 text-sm mb-4">演示 mount 阶段 fetch：自动管理 loading/error/data</p>
...
function RealtimePage(_props: {}, ctx: WfuiContext) {
  // ── mount（只一次）──
  const $ = ctx.ui.$()
  $.messages = []; $.wsInput = ''; $.connected = false

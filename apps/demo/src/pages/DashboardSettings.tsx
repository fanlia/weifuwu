...
export default function DashboardSettings(_props: {}, ctx: WfuiContext) {
  ...
  function handleSubmit(e: Event) {
    ...
    setTimeout(() => {
      saving = false
      ctx.ui.render()
      console.log('设置已保存:', { displayName, email, language, notifications })
    ...
    <div>
      ...
      <form onSubmit={handleSubmit} class="space-y-4">
        <div>
          ...
          <input value={displayName} onInput={(e: any) => { displayName = e.target.value; ctx.ui.render() }}
        ...
        <div>
          ...
          <input value={email} onInput={(e: any) => { email = e.target.value; ctx.ui.render() }}
            class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">语言</label>
          <select value={language} onChange={(e: any) => { language = e.target.value; ctx.ui.render() }}
        ...
        <div class="flex items-center gap-2">
          <input type="checkbox" checked={notifications}
            onChange={(e: any) => { notifications = e.target.checked; ctx.ui.render() }}

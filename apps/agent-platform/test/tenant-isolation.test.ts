...
const EXEMPTIONS: Array<{ file: string; match: string; reason: string }> = [
  ...
  { file: 'src/sandbox/manager.ts', match: 'DELETE FROM sandbox_events WHERE created_at', reason: '保留期事件清理（平台级运维——删除是跨租户统一保留策略（对租户公平），不返回租户数据）' },
  ...
  { file: 'src/services/chat.ts', match: 'SELECT department_id FROM agents WHERE id = ${agent.id}', reason: '产物验证执行归属部门——agent.id 来自运行上下文（已校验的 agent）' },

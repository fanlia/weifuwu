...
test('member/viewer：邀请均 403（Owner only——设置页「仅所有者可用」文案的服务端防线）；报表读面对两角色开放', async () => {
  ...
  const member = await seedRoleMember(BASE, owner, 'member')
  const viewer = await seedRoleMember(BASE, owner, 'viewer')
  const memberInviteForbidden = await tryApi(member, `/api/auth/apps/${owner.app.slug}/auth/invites`, 'POST', { role: 'viewer' })
  const viewerInviteForbidden = await tryApi(viewer, `/api/auth/apps/${owner.app.slug}/auth/invites`, 'POST', { role: 'member' })
  ...
  const memberOwnerInvite = await tryApi(member, `/api/auth/apps/${owner.app.slug}/auth/invites`, 'POST', { role: 'owner' })
  ...
  const adminInviteForbidden = await tryApi(owner, `/api/auth/apps/${owner.app.slug}/auth/invites`, 'POST', { role: 'admin' })

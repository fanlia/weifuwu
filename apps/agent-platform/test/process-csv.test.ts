...
function makeCtx(deptId: string): any {
  return {
    sql: async () => [{ workspace_path: null }],
    _toolDepartmentId: deptId,
  }
}

test('read_csv：注入 _toolDepartmentId → 行数/表头/汇总正确', async () => {
  ...
  const handlers = createHandlers(() => makeCtx(deptId))
...
test('read_csv：无 _toolDepartmentId → 显式「无部门上下文」（不静默）', async () => {
  const { createHandlers } = await import('../skills/builtin/process-csv/tools.ts')

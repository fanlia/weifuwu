...
export function registerDemoRoutes(app: Router<any>): void {
  app.post('/api/demo/space', async (_req: Request, ctx: AppCtx): Promise<Response> => {
    ...
    const { dept, mgr, ai, kb } = await orm.transaction(async (tx) => {
      const T = tables(tx)
      // 1) 部门
      const [dept2] = await T.departments.insert({ app_id: appId, name: '演示项目' }).returning('id').run()
      // 2) 经理（与 departments 自动创建对齐——org-manager 提示词单源）
      const [mgr2] = await T.agents
        .insert({ app_id: appId, type: 'department', name: '演示项目经理', description: '部门经理——代表「演示项目」对外协作',
      ...
      const [ai2] = await T.agents
        .insert({ app_id: appId, type: 'ai', name: '客服小知', description: '演示客服助手',
      ...
      const [kb2] = await T.agents
        .insert({ app_id: appId, type: 'knowledge_base', name: '产品知识库', department_id: String(dept2.id), is_active: true, tools: [] })
        .returning('id')
        .run()
      // 4) 成员
      await T.department_members
    ...
    try {
      ...
      await refreshManagerPrompt(ctx.orm, appId, String(dept.id))
    } catch { /* 刷新失败不阻断 */ }

...
export function registerDemoRoutes(app: Router<any>): void {
  app.post('/api/demo/space', async (_req: Request, ctx: AppCtx): Promise<Response> => {
    ...
    // 1) 部门
    const [dept] = await T.departments.insert({ app_id: String(appId), name: '演示项目' }).returning('id').run()
    ...
    const [mgr] = await T.agents
      .insert({ app_id: String(appId), type: 'department', name: '演示项目经理', description: '部门经理——代表「演示项目」对外协作',
        model: 'deepseek-v4-flash', department_id: String(dept.id), is_active: true, tools: [], allow_file_tools: true })
      .returning('id')
      .run()
    await T.department_members
      .insert({ department_id: String(dept.id), agent_id: String(mgr.id), role: 'manager' })
      .run()
    // 3) 客服 AI + 知识库
    const [ai] = await T.agents
      ...
      .returning('id')
      .run()
    const [kb] = await T.agents
      .insert({ app_id: String(appId), type: 'knowledge_base', name: '产品知识库', department_id: String(dept.id), is_active: true, tools: [] })
      .returning('id')
      .run()
    // 4) 成员
    await T.department_members
      .insert([{ department_id: String(dept.id), agent_id: String(ai.id), role: 'member' }, { department_id: String(dept.id), agent_id: String(kb.id), role: 'member' }])
      .run()

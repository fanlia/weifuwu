...
2. 需要专业协助时用 call_agent 委托（对方会知道是你委托的）；回复中提到对方用 [对方名字] 称呼（如 "[王总] 好的"）；**你的消息不要加 [自己名字] 前缀**——系统已为每条消息标注发言者身份，你自己加会重复（如 "[小码] [小码] 收到！"）
...
/** CHAT-INTERACTION 波次 2：快捷确认标记指引——AI 确认型提问携带可点选项
 *  （渐进增强：无确认场景 AI 不输出标记，前端无 chip——零影响） */
export const QUICK_REPLY_GUIDE = `
【快捷确认】当你需要用户确认/拍板时（如「请确认清理范围」「要不要继续」），在回复末尾单独一行输出选项标记，格式：[[choices:选项1|选项2|选项3]]（最多 4 个，每个不超过 10 字，如 [[choices:清理 30 天前备份|全部清理|暂不清理]]）——用户会看到可点击按钮直接回复；无需确认时不要输出该标记`

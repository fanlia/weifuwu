...
## 2026-12 决策记录：agent 不持有私有空间（保持三层分工）

**决策**（用户 2026-12）：agent 自身不需要私有目录——保持 agent / department / sandbox 分工：
部门 = 工作目录（唯一数据空间），sandbox = 计算资源（共享环境），agent = 纯能力（无数据包袱）。

**理由**：
1. 数据归属保持单一维度（部门）——权限/生命周期/审计/备份/迁移不因 agent 维度复杂化
2. agent 个人状态已有 DB 载体（agent_memories）——文件级个人区非刚需
3. 私有目录引入的「agent × 部门」二维归属收益有限，复杂度代价高
4. 模型纯度：数据只属于部门，能力只属于 agent——新增能力不需要发明新的归属规则

**替代解法**（AI 试错产物不污染共享目录——纪律而非架构）：
- 提示词约定：AI 在部门目录下建 `scratch/` 或 `.tmp/` 临时区试错，交付前清理
- 交付物放部门目录根/显式目录——共享目录整洁靠提示词引导，不靠隔离机制

**不实施**：agent 私有目录（含 .agents/ 路径语义、publish 流程、私有文件浏览器入口、强制隔离 v2 登记）——全部关闭。

## 2026-12 组织层级体验闭环（上级看下属成果）

- [x] **子部门产物可见性**——聚合 API 加 subDepartments（经理代表的部门 + 成员数 + 根目录交付物 top10 只读；排除自我引用）；Chat 右栏「下级部门」区（折叠卡片 + 下载链接）；file_updated → reloadWorkspace 重拉聚合（子部门列表实时化）
- [x] **经理执行身份一致性**——agent-runner：department 类型 agent 按代表部门（agents.department_id）解析工作目录——经理在上级部门被 @ 时在**自己部门目录**干活（真实 AI 验证：管理委员会 @技术部经理 → write 写入技术部目录）
- [x] **提示词实时化**——src/services/org-manager.ts（refreshManagerPrompt：成员名单实时回填）；成员添加/移除路由调用（创建时快照不再过期）
- [x] **组织树可视化**——左栏经理卡显示「代表 XX 部门」；右栏下级部门树（名称/经理/成员数/交付物）
- 验证：浏览器端到端——管理委员会右栏显示技术部交付物（src/README/org-check* 下载）+ 实时刷新 + 经理身份一致性；全量 154/154 + tsc 零错误

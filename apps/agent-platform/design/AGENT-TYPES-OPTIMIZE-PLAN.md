...
| 波次 | 结果 | 交付 | 证据 |
|---|---|---|---|
| W1 经理提示词同步 | ✅ | **缺口重定位**：刷新链路已存在（成员增删路由已接 org-manager）——真实缺口 = 模板三源漂移（departments/agents 内联 vs org-manager）+ agents POST 创建缺成员行/回填——单源收敛 + POST 对齐；契约测试 org-manager 5 项 | 暂无成员→含部门名；加成员→含名（排除经理/user）；移除→回退；幂等；单源审计 |
| W2 部门生命周期 | ✅ | 部门删除补 DELETE 经理 agent（孤儿歼灭） | 契约覆盖（叠加 W1 测试环境） |
| W3 department 详情视图 | ✅ | 代表部门面板（名称/入口/成员名单）+ 模型可保存（创建可配详情此前静默丢弃——假配置歼灭） | UI 测试 dept-manager 2 项（D1 面板渲染/D2 模型持久化） |
| W4 类型单源 | ✅ | AGENT_TYPES 单源（label/icon/color/desc/creatable——TypeBadge/向导卡片/后端白名单全派生）+ 筛选白名单补 department | 契约测试 agent-types 4 项（键集==enum/creatable/无硬编码/完整性） |
| W5 回归门 | ✅ | API 43/43 + UI 37/37 全绿 + tsc 零错 + build OK | 本门复跑 |

**提交**：cea3a1cc（W1/W2）· 9e758ca3（W3/W4）

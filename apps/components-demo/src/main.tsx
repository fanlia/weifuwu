...
const DemoSelect: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.role = '' }
        value={$.role        onChange={e => $.role = (e.target as HTMLSelectElement).value}
        options={[
...
const DemoRadio: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.gender = 'male' }
      <RadioGroup name="gender" value={$.gender} onChange={v => $.gender = v        options={[
          { value: 'male', label: '男' },
...
const DemoForm: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.uname = ''; $.umail = ''; $.submitted = false }
      {$.submitted && <Alert variant="success">表单已提交！</Alert>      <Button type="submit" variant="primary">提交表单</Button>
    </Form>
  return (__props) => {
    )
  }
...
const DemoTable: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.selected = null }
      {$.selected && <div style="font-size:12px;color:var(--wf-color-text-secondary)">已选: {$.selected}</div>    </div>
  return (__props) => {
    )
  }
}
...
const DemoModal: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.open = false }
      <Modal open={$.open} title="确认操作" onClose={() => $.open = false        footer={<Button variant="primary" onClick={() => $.open = false}>确定</Button>}>
...
const DemoToast: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.toasts = [] as ToastItem[] }
    const msgs: Record<ToastType, string> = { success: '操作成功完成', error: '发生了一个错误', warning: '请注意：此操作不可撤销', info: '这是一条提示信息'    $.toasts = [...$.toasts, { id, type, message: msgs[type] }]
...
const DemoAlert: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.showErr = true; $.showInfo = true }
      {$.showInfo && <Alert variant="info" closable onClose={() => $.showInfo = false}>这是一条提示信息（可关闭）</Alert>      <Alert variant="success">操作成功完成</Alert>
...
const DemoCardShowcase: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.clicked = false }
      {$.clicked && <div style="font-size:12px;width:100%;color:var(--wf-color-text-secondary)">卡片被点击了 ✅</div>    </div>
  return (__props) => {
    )
  }
}
...
const DemoFileUpload: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.files = [] as File[] }
        maxSize={5 * 1024 * 1024        value={$.files}
        onChange={f => $.files = f} />
    </div>
  return (__props) => {
    )
...
const DemoDrawer: Component = (_props, ctx) => {
  ...
  $.rightOpen = false; $.leftOpen = false }
      <Drawer open={$.rightOpen} title="编辑用户" position="right" onClose={() => $.rightOpen = false        footer={<>
...
const DemoPopover: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  return (__props) => {
    $.showBottom = false; $.showTop = false }
  }
}
...
const DemoInView: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.log = [] as string[] }
      {$.log.length > 0 && <div style="font-size:12px;color:var(--wf-color-text-secondary)">事件: {$.log.join(', ')}</div>    </div>
  return (__props) => {
    )
  }
}
...
const DemoDatePicker: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.result = '' }
      {$.result && <div style="font-size:12px;color:var(--wf-color-text-secondary);width:100%">已选: {$.result}</div>    </div>
  return (__props) => {
    )
  }
}
...
const DemoEditor: Component = (_props, ctx) => {
  const $ = ctx.ui.$
  $.html = '<p>Hello <strong>weifuwu</strong>!</p><blockquote>引用块示例</blockquote><p style="text-align:center">居中文字</p>' }
        HTML 输出: {$.html?.substring(0, 150) || '(空)'      </div>
    </div>
  return (__props) => {
    )
  }

...
- 主题切换（自动/亮色/暗色）+ 中英切换
- 计数与框架实测同步（113 组件 / 981 测试——style-audit 强制，漂移即红）

## 性能（第二轮优化，2026-12）

- **分组懒渲染**（useInView once-latch）：分组滚入视口才建卡片树，滚走保持（demo 状态不回收）；搜索时强制全渲染，清空恢复懒渲染
- **代码块折叠**：`<details>` 原生折叠（默认收起）+ 复制按钮（copyText）——36% 页面高度退出渲染树
- 实测：首帧 **566ms → 140ms**（-75%）、DOM 节点 **3844 → 194**（-95%）、页面高度 **31 米 → 3 米**

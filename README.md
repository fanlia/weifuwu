...
| 前端(新) | `src/ui-dom/`（试验性独立实现） | UIRouter + VDOM 定稿架构：req=location / res=VNode / serveUI=VDOM / handler=异步组件 / 中间件两阶段 / SSR+hydration——零依赖 `weifuwu/client`（设计见 `design/ui-architecture.md`） |
...
| [docs/frontend-ui-dom.md](docs/frontend-ui-dom.md) | **ui-dom（试验性）**：UIRouter + VDOM 定稿架构——req=location / res=VNode / serveUI / handler=异步组件 / 中间件 / SSR+hydration |

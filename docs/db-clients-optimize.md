...
| 操作 | 自研 | ioredis | 差距 |
|------|------|---------|------|
| set | 0.104ms | 0.084ms | 慢 24% |
| get | 0.122ms | 0.072ms | 慢 70% |
| json 往返 | 0.191ms | 0.109ms | 慢 75% |
| 并发 set | 0.5ms/批 | 0.2ms/批 | 慢 150% |
...
| 目标 | 指标 |
|------|------|
| Redis get/set | 差距收窄到 ioredis 的 ≤1.25×（0.095ms / 0.09ms） |
| Redis 二进制 | getBuffer 任意字节往返相等（含 0x00） |
| PG 大参数 | 256KB jsonb 线性耗时（无 O(n²)），bench 保持反超 |
| PG prepared | LRU 上限 128，长连接 1000 次不同 SQL 后缓存不膨胀 |
| 全量 | 693+ 新增测试全绿，typecheck/build 0 错误 |

...
> 一个 CSS 文件（`weifuwu/components/style.css`）= **115 个双层 Token + 基础层 + 67 个布局原语 + 134 个工具类 + 46 个组件样式 + 5 层 @layer 机制**。
...
├─────────────────────────────────────────────┤
│ 工具类  134 个（间距/边框/圆角/背景/文字/气泡） │ 属性级，任意元素直接写
├─────────────────────────────────────────────┤
│ 原语层  67 个布局原语（stack/row/split…）     │ 元素间的空间关系
├─────────────────────────────────────────────┤
│ 基础层  reset / 原生控件 / focus / 触屏44px / │ 无障碍与默认品质
│         reduced-motion / color-scheme        │
├─────────────────────────────────────────────┤
│ Token   115 个 · 原始层（品牌/中性/暗色值）   │ 值只定义一次
│         语义层（组件消费，主题切换覆盖）       │
└─────────────────────────────────────────────┘
```

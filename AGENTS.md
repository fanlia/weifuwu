...
| R-04 | **小步快跑·单变量探针（2027-11 批量批准 16.8s 挂实录）** | ① **探针最小化**：每步一个假设一个输出（先 `locator.count()` 数匹配 → 再一次点击 → 最后抓请求/toast）——探针 timeout 15s（UI 探针含 server spawn 比 R-01 的 10s 放宽）；**不跑完整测试文件**收窄变量；② **单变量×干净环境**：A/B 必须在**端口级**杀共享 server（`lsof -i:39217 -t \| xargs kill -9`——ps grep 匹配不全）——`getSharedServer` 探测复用**任何**存活进程（手动 nohup 残留以旧代码/无 WF_TEST_HOOKS 毒化全部后续实验结果——探针/测试结果互相矛盾的根因）；③ **探针 API 对齐纪律**：探针必须逐字复刻被测代码的调用——本轮探针全部 `locator('button', {hasText})`（正确）而测试用 `page.click('button', {hasText})`（忽略）——探针永远无法复现测试 bug（API 不一致=复现无效）；④ **catch 吞错 = 隐形失败**：`page.click(...).catch(() => {})` 多匹配 strict violation 被吞 → 16.8s 超时——catch 必须打印错误（`catch(e => console.log('[x]', e))`）再决定吞；⑤ **playwright API 事实表**：`hasText` 是 `locator(sel, {hasText})` 的选项（LocatorOptions）——**`page.click(sel, {hasText})` 运行时静默忽略**（TS 类型也报——溢出到运行时）；CSS `:has-text("批准")` 是 **includes** 语义（批量栏「批量批准选中」误命中实证——改精确正则 `^\s*批准\s*$`）；编程 `el.click()`（evaluate）是第三路径（与 Playwright 坐标 click 行为不同——不能混用互为对照） |

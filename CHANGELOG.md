...
- components：C2-W2 弹层钩子接线——Popover/Tooltip max-width → min(var(--wf-popup-max,…),calc(100vw-32px))
- components：C2-W1 line-height 接线——8 处近值恒等 → var(--wf-line-height-*)
- components：W3 半标尺补面 + 结构值理由定案
- components：W2 标尺档 token 化——42 形态手写 4/8/12/16px → var(--wf-space-*/--wf-gap-*)
- components：W1 发丝 token 化——43 形态 1px 字面量 → var(--wf-border-width)

### Fixed

- layout：W4 token 面定案——双标尺派生 + 同值双名单源化 + px 白名单
- layout：W3 断点单源 + token 死面清理——L10/L11 双哨兵
- layout：W2 层叠语义修正——utilities 恒胜 components + @property 钩子隔离 + display 族 :where
- layout：W1 CSS 装配单源——四管线归一 + 层序声明复活 + 新鲜度键含依赖
- release：changelog 生成接口漂移修复 + v0.92.0 条目补录

### Docs

- components：C2-W4 收尾——token 消费纪律成文 + 快照 + 全量回归门全绿
- components：W4 收尾——组件 px 四纪律成文 + 快照更新 + 全量回归门全绿
- layout：W7 收尾——全量回归门全绿 + 规则生效 + 计划归档
- layout：W5 参考面生成 + 悬空引用清零——L15/L16 双哨兵

### Tests

- components：C2 token 消费哨兵（line-height 近值/弹层面板宽/零消费三桶——W0 防线先行）
- components：C1 组件 px 分类哨兵（发丝/标尺/结构值/z-index 四桶登记制——W0 防线先行）
- layout：W0 防线先行——层叠语义场景契约 + 冲突矩阵/零值档位登记 + CSS 体积基线

### Chore

- components：C2-W3 零消费 83 项定案——全保留 0 删除（理由入档）
...
- W3b MemoryPostgresServer 消亡——postgres-server/servers.test/test-servers 删 + 契约处置（connection/client 真库 gate · memory-pg-platform 改 compileQuery 静态形状 · departments-pilot/gql/orm 转 memory 引擎）——server 域 800/800 绿
- W3a whereRaw 全链删——builder/接口/CHAIN/where 值 raw 执行+编译（memory resolveRawWhere·compileWhere·mergeWhere·WhereField）——测试 3 删 3 改写（db 233·平台非 ui 451 绿）
- W2-ui 播种/断言 AST 化收尾——/api/test/orm + buildQuery 协议面 + whereRaw 清零（非 ui 465 · ui 155 · db 236 全绿）
- W2 非 ui 播种/断言 AST 化——零 SQL 文本面（310 tests 绿）
- W1 DDL AST 化——协议层 = AST（compileSchemaDdl + ddlToSql 双面）
- W1 DDL AST 化——协议层 = AST（compileSchemaDdl + ddlToSql 双面）
- W4 memory 接入收口——ui 测试 155/155 全绿（零 docker）
- W4 平台测试 memory 接入（第二部分）——启动链路全通 + 45 非 ui 文件全绿
- W4 平台测试 memory 接入（第一部分）——postgres({memory:true})/redis({memory:true})
- W2 memory 事务快照回滚——失败可回滚（不再 no-op 直跑）
- W1 未知列校验——两端一致报错（memory 不再静默）
- 恢复 orm 收敛态（零 SQL 兜底）+ raw 取消——细算子 now/nowInterval/colRef
- appAuth 业务侧认证中间件——分离件（_builtin 签发→业务进程验签）
- 路由命名空间统一——/api/apps/{slug}/auth/* + SSO 挂 _builtin 名下
- USERSYSTEM-V4 管理面定案——_builtin=应用管理面·_default=平台业务面·appKey 分离凭据
- USERSYSTEM-V3 系统域定案——_builtin=系统容器·四层角色跨域复用
- USERSYSTEM-V2 W6-W7——useSession 内建 ctx.ui + 平台 auth.ts 收敛删除
- USERSYSTEM-V2 W1-W5——_builtin/registerWithApp/ctx.session/me 会话面/OIDC SSO/角色白名单/members 元数据
- 多模态消费迁移——image-gen/video-gen 编排与 provider 分离（AI-REBUILD W6）
- AIInterface 正名 + MemoryAi/MemoryAiServer + 多模态纳入（AI-REBUILD W2-W7）
- memo 机制（opt-in shouldRender——段级跳过）+ 引用短路 + 边界文档清理
- 健壮性加固四件套（预算/解析深度/响应上限/run 中间态）
- wfjs 源码编辑（编辑/差异预览/保存）+ AGENTS 复用纪律（前后端查库）
- CodeEditor 语法高亮（复用 CodeBlock tokenize）+ CronPicker 平台组件
- def 版本历史 + 回滚——改坏恢复能力（编辑面增长的自然需求）
- 模板教学化 + 列表行内执行——最短用户路径
- try/catch——监控流程核心形态（失败也能通知——JS 逐字对齐）
- 列表质量 + 模板库——管理效率与冷启动体验
- 步骤增删——编辑器完整闭环（表单 ⇄ DSL 双向）
- 步骤参数编辑器——表单视图 ⇄ DSL（patch 单实现）
- LLM 生成质量——create_workflow 带上演示端点上下文
- cron 定时触发全链（解析器/调度器/存储/API/UI）
- 对话生成入口——LLM ⇄ wfjs 全链闭环（三 tool）
- 运行历史可查看结果——步骤级摘要 + 原始 JSON
- 本地 demo 链接 + wfjs 注释/除法缺口修复
- C 收尾——路由层契约（HTTP fixture）+ docs + audit 回归门
- 波次 B——workflow UI（列表 + 三视图详情 + 执行）
- workflowSystem 框架系统——存储/CRUD/执行记录收编框架（对齐 messager）
- 视图适配——workflowToDag/toJsonSchema（组件库零耦合）
- validate 变量自足防线——vars.*/steps.* 引用存在性检查（闭环补全）
- v2 三件——函数提升/块级遮蔽/函数体内函数调用（JS 对齐收官）
- W8b 远程导入 + export——模块层收口（ESM 逐字）
- W8a 函数——function 定义/call 步骤/return 值/递归守卫 64/toJs 对称
- 去重原语整体移除 → std store 显式记账（用户拍板：不发 once 原语，提供 KV 存储代码中使用——JS 一致）
- W6b runner 递归子链执行器 + IR 类型对齐——assign/if(分支)/while/for/return(终止) + ctx.vars + skipped 删除 + edge 静默=继续（97 契约 + e2e 编译产物真执行闭环）
- W6a 表达式层 JS 对齐——===/!== 严格比较 + 三元 ?: + std 纯函数表达式内调用（STD_FNS 注册表）+ 复合赋值 *= /= %= + var 拒绝 + 系统根路径放行（91 契约全绿）
- W5b compileWfjs 编译器——受限 JS 子集→DSL（语句/表达式/绑定映射/静态检查 26 契约）
- W5a 表达式升级——JS 语义子集（宽松比较/大小比较/严格算术防呆/[*] 投影/.length/逻辑返回操作数/JS truthy/裁剪 exists）+ 契约更新 + fuzz 2000 对
- W4 框架接入——exports ./workflow 子路径 + build 独立 bundle + docs/server.md §7 + AGENTS 沙纲登记；回归门七线全绿
- W3 edge 去重状态机（「发一次」全周期：上升沿/静默/解除/再触发）+ ai/email 步骤 + 适配器注入
- W2 执行器——步骤链/ctx 数据流/when 跳过/if 截断/dry 打桩 + validate 闸门 + http/template/log 步骤 + 契约测试
- W1 表达式引擎——path/比较/exists/逻辑/插值（零依赖手写递归下降）+ 契约测试 + fuzz 对账 1500 对

### Fixed

- WEIFUWU_MESSAGER_SCHEMA direct_key 补 nullable——迁移与运行时 shape 漂移（老库 ADD COLUMN NOT NULL 存量 5751 行 23502 启动失败）
- W3c 遗留陈旧文案——报错不再指向已消亡 sql.unsafe（DDL/全表操作指向 migrateModule/runMigration 合法面）
- W1 typecheck 0——client.ts 两分支 migrateModule 归位（真库 ddlToSql / memory 直 executeDdl）
- stats 空聚合组崩溃 + 平台测试 seed 端点（WF_TEST_HOOKS）
- DdlQuery op 联合类型补 alterEnumAddValue（tsc 0）
- 删除优雅关闭（SIGINT/SIGTERM 不注册 handler——默认立即退出）
- pg 连接池默认 50→20——dev 无法访问根因（双实例 50+50 击穿 PG max=100）
- 高度塌缩 + 行号错位（overlay 布局流修复）+ 执行参数 UX 引导
- 三个 tabs 一致性——步骤 tab 实例化 + 详情响应语义化
- fuzz 对账补 toBoolean 导入

### Docs

- orm gql 提升——设计前提（分层纪律）+ 命名契约 + 体验目标 + 波次重构 W0-W6
- orm gql 提升立项——一致性收口 × 可用面补全（探针实证 I1-I5）
- W1/W2/W4/W5 完成勾选——计划全波次收尾
- W3 完成勾选（状态机化落地——c4b045c3）
- §3.5 orm 架构纪律——状态机管理原则（三组件状态确定性/一致性铁律/单源/盲区警惕）
- orm 状态机管理——三组件状态确定性（shape 单态冻结/operator 纯函数/adapter 真状态机）+ W3 机制化（转换守卫+行为矩阵+inspectTable）
- 确定性原则机制化——三层审计（S1-S6/O1-O5/A1-A5）+ 波次重排（O1 eq:null 语义分裂·A1 jsonb 解码残差·S3 softDelete 零行为定案）
- orm 覆盖补全立项——理论矩阵×现实场景交叉（tenant 0 使用/45 处手写 app_id·transaction 0/16 多写文件·enum 坍缩）
- ctx.sql 文档残留清零——postgres() ctx 面标注 ctx.orm（历史计划注记同步）
- orm DX 计划收尾——W4 实录 + 验收全勾选（五域 1722 全绿·3 真 bug 实证清单）
- §5.1 typedQuery 用法 + §5.2 shape satisfies 纪律（平台 shapes 指南——禁止 ZodRawShape 注解）
- W3 实录——typedQuery 类型机制收敛 + RosterMember roleLabel 真 bug（运行时 undefined 实证）
- W2 实录——satisfies 29 表 + sandboxes updated_at 三单源漏列实证（memory 掩盖真库炸点）
- W1 实录——类型守卫面 220→0（String 遮蔽/ZodEnum 坍缩/aggregate 键缺口登记 W2/W3）
- orm DX 类型完整体——探针 P0-P4 实录 + 根因定位（SHAPES ZodRawShape 注解坍缩 Infer / tsd 断言无守卫 / 跨表无类型）
- orm-协议AST化 收尾——W4 实录 + 验收勾选（五域+七线全绿）
- W3b 实录
- W3a 实录——whereRaw 删除清单 + 测试处置 + 数字（233/451）
- W3 探针——unsafe 146 行/14 文件清册 + 消费者分类（parser/whereRaw/MemoryPostgresServer）
- 路由语义定案——/api/apps/ 为 _builtin 专属·业务路由应用层自决
- USERSYSTEM-V2 完成计划归档 + AGENTS.md 防线快照更新（433/737/328/123/25/449）
- 外部依赖内存化矩阵登记——四类核心依赖 Memory 实现 + Server 协议替身双层（含测试铁律：禁自建 fake）
- AI-REBUILD 计划完成归档（规则入 docs/server.md §3.5 + AGENTS.md—— 历史计划 git 承接——工作区不保留完成态）
- vdom 渲染增强计划完成收尾（实录/判负入 git 历史——本目录只留进行中）
- §7 裁剪清单更新（v2 三项转正）
- §7 语义红线对齐现状（表达式 JS 子集/函数/store 去重/远程导入）+ plan 实录 W8b

### Tests

- W3 fuzz 值面算子对账——mergeInc/mergeAppend 随机序列 vs 数学期望
- fake server 全清——统一 Memory 系 Server 协议替身（用户拍板）
- 多模态 env 默认契约——零显式配置走 DASHSCOPE_MAAS_API_URL + DASHSCOPE_API_KEY + 默认模型常量
- 对话生成→执行链 e2e——MemoryAiServer 协议替身实战（AI-REBUILD W9）
- /workflows 列表页 UI 全流程测试（7 用户故事）+ 删除功能真实 bug 修复

### Chore

- plan 实录 W8a
- plan 实录去重原语裁决（store 显式记账）
- plan 实录 W6a/W6b
- plan 实录 W5a/W5b

### Other

- W5 收口：验收标准全达成（orm-体验提升 计划闭环）
- W5：docs §5.3 增补 + 全量回归门全绿
- W4：平台试点——agents POST bodyOf 收口 + listQuery 手写解析消亡
- W3：sort 类型化 + checkConsistency 诊断（启动对齐面）
- W2：undefined 值面定案——双路径统一显式拒绝（不静默·不分裂）
- W1：listQuery + errorResponse 共享提取（rest 私有 → 协议面 helpers）
- W0：bodyOf —— shape → body 校验面（handler 样板消亡第一刀）
- 修订：orm 体验提升——开发者动线主轴（parseBody/listQuery/errorOf 样板消亡 + 正确性收口）——平台摩擦盘点锚定（23 body/32 URL/26 错误映射）
- 立项：orm 体验提升——价值面一致性 × 类型化收口 × 诊断面（探针锚定——P0 undefined 双路径分裂实证）
- plan/orm-gql-提升.md：W0-W6 执行实录收口（波次闭环 + 回归门全绿）
- W6：docs §5.4 完整（用法/边界判别/三条铁律/判负清单/执行实录）+ 回归门
- fix：/api/gql 接线过早注册 + ui 共享 server 失败路径泄漏（npm test 永不结束根因）
- W5：平台试点——/api/gql + REST 面（第二消费者）+ 试点的执行实录修正
- W4：restFromShape —— RESTful 路由组生成（样板 handler 分层纪律）
- W3: 分页 total——ListPage（rows+total 单面——与 paginate 对齐）
- W2: enum filter 值面 + vector 字段呈现——I4/I5 定案（不透明归零）
- W1: 一致性收口——I1 eq:null 双端 / I2 sort 多字段链 / I3 orm.gql 租户单源
- W0: 命名契约 + 共享地基——filterToWhere 提取 + fieldPolicy.hidden
- W5: 分页试点 + docs §5.3 确定性契约 + 全量回归门
- W2: 事务面平台收口——survey campaign + demo space 试点原子化（1+N/7 写→单链回滚）
- W1: 租户 scope 接线——options.tenant → 中间件 ctx.orm 自动 CtxOrm + 惰性求值修复
- W4: enum 单源 + vector 声明面——字面量推断复活 + 值面列型绑定（S1/S2 定案）
- W3: 状态机化——MemoryTable 显式状态 + 转换守卫 + 行为矩阵（确定性原则落地）
- W3 试点余量: messages 返回行断言删（T.messages 绑定 RowOf 精确——W2 面顺带）
- W3: createTypedQuery 跨表类型化——模板字面量列解析 + 平台试点（2 真 bug）
- W2: weifuwuAppMembers 同 satisfies 化（shape 注解类型一致性）
- W2: 单表类型复活——SHAPES ZodRawShape 注解 → satisfies（23+6 表字面量保留）
- W1: 类型契约真守卫——tsconfig.test.json 引入测试域 typecheck（220→0）
- docs/audit: W4-1 数据层协议生效规则 + audit-orm 双范围 0 升级
- W3c: 框架 SQL 文本面删净——parser/unsafe/tag 消亡，协议层 = AST
- refactor(db/ai/email): Memory 系 Server 正名补齐——完整四象限主包导出
- refactor(email): 对齐 AI 模块——MemoryEmail/MemoryEmailServer + API 发送（删 SMTP）
- refactor(ai): DASHSCOPE_MAAS_API_URL 已是完整 https URL——去掉协议补全逻辑（baseOf 直用）
- refactor(ai): 多模态拆包——image/video 独立配置（同 embedding 平级——多 url 多 key）
- refactor(ai): ai() 全库清除——测试/消费全部迁移 OpenAi 正门构造
- refactor(ai): 清除 ai() 选择器残留（index.ts 纯 re-export——正门 OpenAi/MemoryAi）
- refactor(ai): 移除 ai() 选择器——正门 OpenAi/MemoryAi（用户拍板：不再导出 ai 模块）
- refactor(ai): provider 正门构造对称——new OpenAi() / new MemoryAi()（同 ai() 返回模块）
- refactor(ai): W1 AIInterface 正名 + 多模态三方法 + Context 声明单源（Ai 兼容别名）
- refactor(workflows): routes 参数默认化——appId 提取器走框架 ctx.auth.appId（user 会话透传）

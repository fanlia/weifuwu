...
**执行状态（2026-12）：裁剪（评估后不实施）**——理由：
1. diff:transition 决策事件已在阶段 0 发射（from/to kind——**观测覆盖已达成**）
2. KIND_PATCHERS 查表已存在（同 type——native/comp/app/frag/portal/text/null）
3. 异 type 统一重建正确（阶段 1 占位法对称——空洞↔真实显式互换）
4. 矩阵显式化（7×7 每组合函数）边际收益低（多数组合同"重建"语义——
   显式化重复代码）——**裁剪登记**（如需转换语义特殊化——未来按需单点加）

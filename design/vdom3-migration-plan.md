...
| D6 | **转正发布 = minor 版本**（用户决策 2026-08：不跳大版本——继续 minor 演进）——默认入口切 vdom3 | vdom2 冻结保留（git 历史完整）；兼容性由 minor 语义管理 |
...
| 里程碑 | 内容 | 估时 |
|--------|------|------|
| M1 | 阶段 0-1：差距冻结 + 引擎补齐 | ✅ 完成 |
| M2 | 阶段 2：hooks 兼容层（最大项） | ✅ 完成 |
| M3 | 阶段 3：组件库全绿 | ✅ 完成（零迁移） |
| M4 | 阶段 4：生产机制（SSR/motion/空洞对齐——audit 剩余） | 🔄 |
| M5 | 阶段 5-6：应用迁移 + 退役 + **minor 发布**（不跳 1.0） | ⬜ |
| **总计** | | **阶段 0-4 完成；5-6 待执行** |

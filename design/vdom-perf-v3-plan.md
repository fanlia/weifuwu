...
> 状态：规划中（2026-08）· v2 已闭环（P-1~P-6 + S-1 定论 + 文档引导）
...
| 指标 | v2 基线 | v3 目标 | 来源 |
|---|---|---|---|
| build 更新（native 1000 行剪枝） | 13.4ms | 2-3ms | V3-2 |
| patch 更新（native 1000 行） | 31-41ms | 20-25ms（文本/引用短路） | V3-1 + V3-3 |
| 文本节点更新（单文本） | replaceChild ×2 | nodeValue ×1 | V3-1 |
| 流式追加（文本密集） | 30（3.4ms/帧） | ~20 | V3-1（文本节点复用） |

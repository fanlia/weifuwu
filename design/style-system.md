...
> 一个 CSS 文件（`weifuwu/components/style.css`）= **182 个双层 Token + 基础层 + 66 个布局原语 + 157 个工具类 + 48 个组件样式 + 5 层 @layer 机制 + 3 套预设主题**。
...
├─────────────────────────────────────────────┤
│ 原语层  66 个布局原语（stack/row/split…）     │ 元素间的空间关系
├─────────────────────────────────────────────┤
│ 基础层  reset / 原生控件 / focus / 触屏44px / │ 无障碍与默认品质
│         reduced-motion / color-scheme        │
├─────────────────────────────────────────────┤
│ Token   182 个 · 原始层（品牌/中性/暗色值）   │ 值只定义一次
│         语义层（组件消费，主题切换覆盖）       │
└─────────────────────────────────────────────┘
```

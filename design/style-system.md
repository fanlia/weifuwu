...
> 一个 CSS 文件（`weifuwu/components/style.css`）= **166 个双层 Token + 基础层 + 57 个布局原语 + 136 个工具类 + 48 个组件样式 + 5 层 @layer 机制**。
...
├─────────────────────────────────────────────┤
│ 原语层  57 个布局原语（stack/row/split…）     │ 元素间的空间关系
├─────────────────────────────────────────────┤
│ 基础层  reset / 原生控件 / focus / 触屏44px / │ 无障碍与默认品质
│         reduced-motion / color-scheme        │
├─────────────────────────────────────────────┤
│ Token   166 个 · 原始层（品牌/中性/暗色值）   │ 值只定义一次
│         语义层（组件消费，主题切换覆盖）       │
└─────────────────────────────────────────────┘
```

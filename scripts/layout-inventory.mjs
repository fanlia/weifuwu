 ...
 *
 * 消费方：src/level5/client/layout/style-audit.test.ts（计数/组合防线）、docs 计数同步。
 */
...
export async function inventory() {
  ...
  const { generateLayoutCss } = await import('../src/level4/client/layout/define.ts')
  const { structures } = await import('../src/level4/client/layout/decl.ts')
...
function collectCorpus() {
  ...
  for (const r of ['src', 'docs', 'design', 'README.md']) walk(join(root, r))
  _corpus = corpus
  return corpus
}
...
export function componentInventory() {
  const compDir = join(root, 'src/level5/client/components')

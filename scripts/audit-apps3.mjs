// 应用模板本体巡检（正确启动：cd examples/apps/:id && node server.ts）
...
async function check(dir, port, fn) {
  ...
  s.kill(); await new Promise(r => setTimeout(r, 400))
}

// todo
await check('todo', 3300, async () => {
  const t = await page.evaluate(() => document.body.textContent ?? '')
  log(t.length > 100, `todo 启动（内容 ${t.length} 字符——${/任务|待办/.test(t) ? '任务渲染✓' : '✖'}`)
...
await check('multi', 3303, async () => {
  ...
  log(t.length > 100, `multi 启动（内容 ${t.length} 字符——按钮: ${btns}）`)
})

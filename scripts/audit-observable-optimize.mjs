...
// ── 检查 1：调度器无隐式时序 ──
const sched = read('src/level1/vdom/v2/schedule.ts')
...
// ── 检查 2：组合算子面 ──
const idx = read('src/level1/vdom/observable/index.ts')
...
// ── 检查 3：失败可观测 + 派生单源 ──
const envSrc = read('src/level3/vdom/hooks/env.ts')
if (!/asyncErrors\$\.next/.test(envSrc)) violations.push('useAsyncData 错误未入流（asyncErrors$ 缺失）')
const storeSrc = read('src/level1/vdom/store.ts')
if (!/export function derived/.test(storeSrc)) violations.push('derived 单一实现源缺失（store.ts）')
const derivCount = (read('src/level6/client/vdom/index.ts').match(/\bderived\b/g) ?? []).length

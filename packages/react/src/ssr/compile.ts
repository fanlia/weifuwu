/**
 * compile — ESBuild-based TSX/TS module compiler for React SSR.
 *
 * Dev mode: per-file transformSync (~0.5ms) + shared vm context via server-registry.
 * Prod mode: bundled ESM via esbuild.build, cached and deduplicated.
 */
...
export async function compileVendorBundle(): Promise<string> {
  ...
  const reactAbsPath = isBundled() ? resolve(baseDir, 'react.js') : resolve(baseDir, 'ssr', 'react.ts')

...
export async function runAgentRouting(
  ...
  for (const am of agentMembers) {
    ...
    if (stream) {
      // ── Streaming mode ──────────────────────────────────────────
      agents.run(agentId, { input: content, messages: history, stream: true }).then(async (result) => {
      ...
      agents.run(agentId, { input: content, messages: history, stream: false }).then(result => {

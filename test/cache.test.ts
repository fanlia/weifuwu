...
describe('cache', () => {
  it('returns fresh response on first request', async () => {
    ...
    const r = new Router().use(c).get('/data', () => {
      count++
  ...
  it('does not cache POST requests', async () => {
    ...
    let count = 0
    const c = cache({ store: mem, ttl: 60_000 })
    const r = new Router().use(c).post('/data', () => {
      count++
      return Response.json({ count })
    })

    await r.handler()(new Request('http://localhost/data', { method: 'POST' }), {
      params: {},
      query: {},
    } as any)
    const res = await r.handler()(new Request('http://localhost/data', { method: 'POST' }), {
      params: {},
      query: {},
    } as any)
    assert.equal(((await res.json()) as any).count, 2)
  ...
  it('does not cache requests with Cookie header', async () => {
    ...
    await r.handler()(new Request('http://localhost/data', { headers: { cookie: 'foo=bar' } }), {
      params: {},
      query: {},
    } as any)
    const res = await r.handler()(
  ...
  it('does not cache responses with Set-Cookie by default', async () => {
    ...
    const r = new Router().use(c).get('/data', () => {
      count++
      return new Response('ok', { headers: { 'set-cookie': 'sess=abc' } })
    })
  ...
  it('respects cacheStatus option (caches 404)', async () => {
    ...
    const r = new Router().use(c).get('/nf', () => {
      count++
      return new Response('not found', { status: 404 })
    })

    await r.handler()(new Request('http://localhost/nf'), { params: {}, query: {} } as any)
    const cached404 = await r.handler()(new Request('http://localhost/nf'), {
      params: {},
      query: {},
    } as any)
  ...
  it('does not cache binary content types', async () => {
    ...
    let count = 0
    const c = cache({ store: mem, ttl: 60_000 })
    const r = new Router().use(c).get('/img', () => {
      count++
      return new Response('fakeimg', { headers: { 'content-type': 'image/png' } })
    })
  ...
  it('sets X-Cache header on cached responses', async () => {
    const mem = new MemoryCache()
    const c = cache({ store: mem, ttl: 60_000 })
    const r = new Router().use(c).get('/data', () => new Response('ok'))
  ...
  it('different URLs have separate cache entries', async () => {
    ...
    const r = new Router()
      ...
      .get('/a', () => {
        aCount++
        return Response.json({ val: 'a' })
      })
      .get('/b', () => {
        bCount++
        return Response.json({ val: 'b' })
      })
  ...
  it('query params differentiate cache keys', async () => {
    ...
    let count = 0
    const c = cache({ store: mem, ttl: 60_000 })
    const r = new Router().use(c).get('/search', (req, ctx) => {
      count++
      return Response.json({ q: ctx.query.q })
    })

    await r.handler()(new Request('http://localhost/search?q=foo'), {
      params: {},
      query: { q: 'foo' },
    } as any)
    await r.handler()(new Request('http://localhost/search?q=bar'), {
      params: {},
      query: { q: 'bar' },
    } as any)
    assert.equal(count, 2)
  ...
  it('respects Cache-Control: no-store from response', async () => {
    ...
    let count = 0
    const c = cache({ store: mem, ttl: 60_000 })
    const r = new Router().use(c).get('/ns', () => {
      count++
      return new Response('no', { headers: { 'cache-control': 'no-store' } })
    })
  ...
  it('MemoryCache cleanup removes expired entries', async () => {
    ...
    await quick.set(
      'k1',
      { status: 200, statusText: 'OK', headers: {}, body: 'x', createdAt: Date.now(), tags: [] },
      30,
    )
    await quick.set(
      'k2',
      { status: 200, statusText: 'OK', headers: {}, body: 'y', createdAt: Date.now(), tags: [] },
      30,
    )
  ...
  it('HEAD requests skip cache (no body to cache)', async () => {
    ...
    let count = 0
    const c = cache({ store: mem, ttl: 60_000 })
    const r = new Router().use(c).head('/data', () => {
      count++
      return new Response(undefined, { headers: { 'x-count': String(count) } })
    })

    const res1 = await r.handler()(new Request('http://localhost/data', { method: 'HEAD' }), {
      params: {},
      query: {},
    } as any)
    assert.equal(res1.headers.get('x-count'), '1')

    const res2 = await r.handler()(new Request('http://localhost/data', { method: 'HEAD' }), {
      params: {},
      query: {},
    } as any)

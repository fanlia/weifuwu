...
describeRedis('rateLimit with Redis store', () => {
  ...
  it('uses custom key function', async () => {
    rl = rateLimit({ max: 1, window: 60_000, store: 'redis', redis: redisClient.redis, key: (req) => req.headers.get('x-api-key') ?? 'anon' })
    const r = new Router()
      .use(rl)
      .get('/data', () => new Response('ok'))
  ...
  it('custom message for 429', async () => {
    rl = rateLimit({ max: 1, window: 60_000, store: 'redis', redis: redisClient.redis, message: 'Custom 429' })
    const r = new Router()
      .use(rl)
      .get('/data', () => new Response('ok'))

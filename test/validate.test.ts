...
describe('validate', () => {
  ...
  it('validates query params', async () => {
    ...
    const res = await r.handler()(new Request('http://localhost/search?q=hello&page=2'), {
      params: {},
  ...
  it('validates params', async () => {
    ...
    const res = await r.handler()(new Request('http://localhost/507f1f77bcf86cd799439011'), {
  ...
  it('rejects invalid params with 400', async () => {
    ...
    const res = await r.handler()(new Request('http://localhost/bad-id'), {
      params: { id: 'bad-id' },
      query: {},
    } as any)
    assert.equal(res.status, 400)
  ...
  it('passes through when no schemas provided', async () => {
    const r = new Router().post('/data', validate({}), () => new Response('ok'))
  ...
  it('rejects invalid headers with 400', async () => {
    ...
    const res = await r.handler()(new Request('http://localhost/check', { headers: {} }), {
      params: {},
      query: {},
    } as any)
    assert.equal(res.status, 400)
  ...
  it('keeps raw string body for text/plain Content-Type', async () => {
    const r = new Router().post('/data', validate({ body: z.string() }), (req, ctx) =>
      Response.json({ body: ctx.parsed?.body }),
    )
  ...
  it('skips body validation for GET method even with body schema', async () => {
    ...
    const res = await r.handler()(new Request('http://localhost/data', { method: 'GET' }), {
      params: {},
      query: {},
    } as any)
    assert.equal(res.status, 200)
  ...
  it('preserves existing ctx.parsed values from upstream middleware', async () => {
    const r = new Router()
      ...
      .post('/data', validate({ body: z.object({ value: z.number() }) }), (req, ctx) =>
        Response.json(ctx.parsed),
      )
  ...
  it('parses form body without Zod schema when validate() has no schemas', async () => {
    const r = new Router().post('/contact', validate(), (req, ctx) =>
      Response.json(ctx.parsed?.body as Record<string, string>),
    )
  ...
  it('URL-encoded form with duplicate keys uses last value', async () => {
    const r = new Router().post('/data', validate(), (req, ctx) =>
      Response.json(ctx.parsed?.body as Record<string, string>),
    )
  ...
  it('skips body validation for HEAD method', async () => {
    const r = new Router().head(
      '/data',
      validate({ body: z.object({ name: z.string() }) }),
      () => new Response('ok'),
    )

    const res = await r.handler()(new Request('http://localhost/data', { method: 'HEAD' }), {
      params: {},
      query: {},
    } as any)
    assert.equal(res.status, 200)

# SaaS 能力指南（auth/limit/email/queue/cron）

> 从 docs/saas.md 迁移（content/ 文档库——随 npm 包发布，与框架版本同步）。
...
> ⚠️ **weifuwu/client 已删除**——前端运行时唯一入口为 `weifuwu/ui-dom`，见 [ui-dom 指南](ui-dom-guide.md)。
...
// 流式对话：路由一行返回 SSE（wf: 协议，详见 [ai-contract](../../docs/ai-contract.md)）
...
- **协议**：`wf:` 命名空间（message_start/token/tool_call/tool_progress/usage/done/error + agent 扩展 step/approval_request），SSE 下行 + POST 上行，错误即值、未知事件透传、`x:*` 自定义事件（详见 [ai-contract](../../docs/ai-contract.md)）
